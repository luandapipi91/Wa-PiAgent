...
- v0.6.4 发版：升版 0.6.3 → 0.6.4（2 提交：bfea2761 浏览器工具 UA 去 headless + 收编在途 eb76a094 关闭 Chrome 自动化标志）。内容：内置浏览器访问反爬站点不再被认出（UA 与 navigator.webdriver 均按普通电脑）。验证：browser 域测试 24 pass；四包 typecheck + kernel 全量回归 + 双端打包 gate。
