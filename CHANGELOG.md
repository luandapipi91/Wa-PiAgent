...
- 管控：`packages/kernel/package.json` 的 `version` 从 `0.0.0` 改为 `0.1`（独立于 app 版本的内核版本线），作为内核版本的**唯一管控源**，与产品版本解耦。
...
- 注意：内核版本为**独立版本线**，与 app 版本解耦；需要时独立改 `packages/kernel/package.json` 的 `version` 即可。
