...
- perf(shared+kernel): 委派提示词优化——fleet 选择正确率 88%→100%（连续 2 轮 fleet 20 全对）且委派相关提示词压缩至 591 tok（≤600 预算；DELEGATE 312 + FLEET 43 + 系统委派段 236）：①三层去重（系统段 few-shot 5→2、Task Contract 并入工具描述、FLEET 砍与 DELEGATE 重复的判据）；②判定改为「先查顺序词（先…再…/然后/按…结果/取决于）→ 一律逐个 delegate，禁止 fleet；无依赖且 ≥2 个互不依赖对象（两份文件/多目录/两组并行）→ 一次 fleet 并行；单对象 → delegate(Explore)」；③8 轮迭代（75→88→88→81→94→94→100→100%，轨迹 ~/.wa-pi/eval-dt-iterations.md）后连续 2 轮 16/16；全量 160 条验证：fleet 类 16/16（漏派 0/误派 0）、simple 0/20 误派、基线抽样 edit-small 保持 0 误派，其余类别仅报告（hiagent 误派 22%，优于基线 50%）；报告 ~/.wa-pi/eval-dt-fleet-r7.json、eval-dt-fleet-r8.json、eval-dt-final-full.json
