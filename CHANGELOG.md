...
- **连接测试按钮不再在已连接时隐藏**：`McpCard` 原用 `{status !== "connected" && ...}` 隐藏已连接服务器的「连接测试」按钮，导致已连接后无法主动重测。现始终显示该按钮（测试中时显示「测试中...」并禁用），用户可随时重新连接测试。
  - **影响范围**：frontend(McpCard.tsx 移除 connected 隐藏条件；McpCard.test.tsx 将「connected 不显示」断言反转为「connected 仍显示」)。全量 355 pass / 0 fail，typecheck clean。
