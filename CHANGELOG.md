## 2026-09-16 — v0.4.2 发版（三层记忆系统重构）

- 版本：0.4.1 → 0.4.2（39+ 提交）。
- 重点：记忆系统重构为 3 层架构（L1 常驻画像 / L2 知识 / L3 执行时间线），存储由 markdown 迁至 SQLite + FTS5/BM25（中文 bigram 分词），新增 memory_search 工具与检索端点，存量数据启动时幂等迁移。
- 新增：记忆面板搜索（服务端 FTS、归档 Tab 独立检索、按层筛选、三段式空态）；收缩态 TUI 队列可拖动并持久化。
- 修复：文件树大目录卡顿（虚拟滚动）；长中文会话压缩估算按 CJK 加权（防 400 卡死）；带附件消息「重新发送」不丢附件；md 预览虚拟滚动容器错位；TUI 面板展开态标题异常。
- 验证：typecheck 全绿；四层回归全绿（隔离 worktree）。
- 影响范围：kernel（memory/SQLite DAO/路由/提示词）、shared、frontend（记忆面板/TUI/文件树/预览/消息）。

## 2026-09-16 — feat(memory): 记忆面板收敛——移除旧分类维度 + 搜索接通服务端 FTS/BM25

- 问题（两处欠账）：①旧 markdown 时代的 `category`（记忆/用户/失败）与新的三层 `kind`（画像/知识/执行）在同一工具栏并列，7 个筛选按钮里「失败」在 DB 模式下无任何写入来源、点了恒为空列表；②搜索框仍是本地 `text.includes`（只搜已加载列表），内核已实现的 FTS5/BM25 检索前端零调用，归档 Tab 的搜索框输入后完全无反应（静默失效）。
...
- 检索中提示：由行内文字改为与「没有匹配的记忆」同规格的空态（`MemoryEmpty type="searching"`：🔍 + 「检索中：{词}」+ 等待提示），组件测试与 E2E 同步（E2E 用 `page.route` 延迟放行制造在途窗口）。
...
## 2026-09-16 — feat(frontend): 记忆检索中状态改为三段式空态（含搜索词）

- 需求：记忆页搜索在途时只有一行小字「检索中…」，改为与「没有匹配的记忆」同款三段式空态：🔍 圆角方块 icon 保留 + 标题「检索中：{搜索词}」+ 副文案「正在检索记忆，请等待……」。
- 实现：`MemoryEmpty` 新增 `type="searching"`（复用 search 分支的 72×72 方块规格；标题 i18n 插值 query，query 为空回退「检索中…」）；`MemoryPage` 检索在途分支换用该组件，外层保留 `data-testid="memory-search-status"`（既有测试断言不变）；i18n zh/en 新增 `memoryEmpty.searchingTitle/searchingHint`。
- 四层测试：组件测试补断言（memory-empty-searching + 「检索中：pnpm」+ 提示语，tests/MemoryPage.test.tsx 检索在途用例）+ E2E 1 例（route 拦截延迟放行制造在途窗口：断言三段式可见、放行后让位给命中列表，e2e/memory.spec.ts）。
- 验证：tsc 退出 0；bun run build 成功；MemoryPage 组件测试 26 pass / 0 fail；E2E memory.spec.ts 12 passed。
- 影响范围：packages/frontend/src/components/memory/MemoryEmpty.tsx、MemoryPage.tsx、src/i18n/locales/zh.ts、en.ts、tests/MemoryPage.test.tsx、e2e/memory.spec.ts。
...
- 验证：`tsc --noEmit` 全绿；AutoWidthSelect + ModelSelector + ThinkingSelector + ComposerInput 合跑 27 pass 0 fail，new-session 三类合跑 15 pass 0 fail；`bun run build` 后确认 Tailwind 已生成新增工具类（appearance-none / max-width:240px / width:0 / min-width:100% / justify-self:end / padding-right:1.25rem）；箭头尺寸在真实 Chromium 中用等价 CSS 隔离页逐档量测（path.getBBox 换算可见宽高 + 描边像素 + 文字间距），并与原生 select 同屏对比取最接近的一档；长名 495→240 截断、行内零溢出、两个下拉箭头完全同一枚 path。已安装桌面版需重新打包才能看到（打包版自带前端 bundle），dev 模式热更新直接生效。
- 影响范围：packages/frontend（src/components/ui/AutoWidthSelect.tsx 新增、src/components/ui/ModelSelector.tsx、src/components/ui/ThinkingSelector.tsx、src/components/ui/AutoWidthSelect.test.tsx 新增、tests/ModelSelector.test.tsx）。
...
- 版本：0.3.15 → 0.3.16。
- 修复：会话自动归档误伤（恢复重删/新会话误归档/并发写覆盖/原子写/孤儿仅清占位）。
- 优化：会话 Git 工具栏收敛为分支 chip 单入口。
- 验证：typecheck 全绿；四层回归全绿（隔离 worktree）。
- 影响范围：kernel（project-store/agent-manager/ws-server）、frontend（git 工具栏组件）。
...
## 2026-09-09 — fix: 自定义供应商 thinking 模型多轮对话 400（reasoning_content 未回传）

- 修复：用户手动添加的 OpenAI 兼容供应商（如 OpenCode Go / opencode.ai/zen/go）下，reasoning 模型（deepseek-v4-flash 等）多轮对话报 400 `The reasoning_content in the thinking mode must be passed back to the API`——该网关要求 assistant 历史消息回传 reasoning_content。pi 内置目录为 opencode-go 模型显式声明了 `compat.requiresReasoningContentOnAssistantMessages: true`，但 wa-pi 生成供应商扩展时丢弃 compat 字段；pi 引擎的自动探测（detectCompat）仅对 deepseek 官方端点开启该兼容，opencode.ai 不命中 → 多轮请求不回传 → 400（首轮正常、带历史 assistant 消息的后续轮次必挂）。
- 修复：provider-extension 生成链路透传内置目录模型 compat——CatalogModel/SdkModelInfo 增加 compat 字段并透传到生成的模型对象；与既有自建网关 developer-role 修正合并输出（目录 compat 在下、端点修正覆盖在上），空对象时不输出字段；EXTENSION_GENERATOR_VERSION 2 → 3（stale 判定强制重生成已装设备的旧 extension）。
- 验证：TDD——新增「目录 compat 透传（OpenCode Go 回归）」与「目录无 compat 且官方端点不输出字段」用例，自建网关用例断言更新为新格式；provider-extension 45 pass、关联 provider 测试 28 pass、typecheck 绿。
- 影响范围：packages/kernel（src/pi-catalog.ts、src/provider-extension.ts、tests/provider-extension.test.ts）、CHANGELOG.md。生效需随下次内核发版。
...
- 修复（画廊两个实测问题）：① 画廊弹窗图片不居中——ZoomableImage 根节点缺 w-full，作为 flex 子项只占内容宽，居中对齐无效；② 画廊无法切换其他图片——模型在 Windows 上写反斜杠路径，collectMediaItems 的清单 src（原始反斜杠）与 MarkdownImage 比较口径（parseFilePath 正斜杠）不一致导致定位失败退化为单媒体；清单 src 统一归一正斜杠，三处定位比较同步归一。验证：反斜杠场景单测 2 用例 + E2E 反斜杠芯片注入 1 passed，frontend 全量回归绿。
...
- 根因：provider-extension.ts 里 maxTokens 与 contextWindow 同用「目录优先」取值，但目录标称的输出上限对第三方端点不适用；且 stale 判定只比 mtime，升级后模板修复无法落地到已生成的 extension 文件。
- 修复：① maxTokens 改为用户显式配置（>0）优先，目录值仅做缺省回退，与 resolveEffectiveBaseUrl 的「用户显式配置优先」哲学对齐；contextWindow 维持目录优先不动（防压缩回归的既有语义）。② 生成文件头部增加 generator-version 标记（当前 2），stale 判定发现版本缺失/低于当前值即强制重生成，保证升级后模板修复自动生效。
- 验证：provider-extension.test 新增 4 用例（用户值覆盖目录值、未配置回落目录值、旧版本标记/无标记触发重生成、生成头部含版本标记）；全文件 44 pass；关联 5 个测试文件 56 pass；kernel typecheck 通过。
- 影响范围：packages/kernel/src/provider-extension.ts、tests/provider-extension.test.ts。

## 2026-09-02 — fix(frontend): 压缩期间排队消息自动发出后不在界面显示

- 问题：压缩（compact）进行中发送的消息进入排队队列（仅顶部队列面板显示，不进消息列表），压缩结束后 kernel 自动 drain 发出（pi 收到并回复），但消息流界面上这条用户消息消失、只剩 assistant 回复。
- 根因：① drain 是 kernel 内部直接 client.prompt，不产生 session:echo_user（只在 WS agent:prompt 路径合成），该消息在界面上的唯一写入机会是 pi 的 message_start(user) 回声 append；② compaction_end 触发 refreshTokenTotals，GET 读磁盘 jsonl（快照读取早于排队消息落盘）后整表覆盖消息列表，GET 响应晚于回声 append（长会话几乎必然）→ 刚显示的用户消息被冲掉，且无任何补回机制。
- 修复：refreshTokenTotals 覆盖时保留本地比服务端快照新的消息（以快照最后 timestamp 为界合并，timestamp 相同视为服务端已含不重复保留，后续拉取自然收敛）——消除 GET 在途竞态，对正常路径无副作用。
- 验证：store-session.test 新增「覆盖时保留本地比服务端快照新的消息」用例（修复前红灯：排队消息被冲掉；修复后绿灯：保留且历史不重复）；store-session 全文件 85 pass；frontend 全量 2079 pass。
- 影响范围：packages/frontend/src/store/session.ts（refreshTokenTotals）、tests/store-session.test.ts。

## 2026-09-02 — fix(kernel): 空闲回收后的会话切换角色必失败一次（「会话已清理」）

- 问题：会话被空闲回收（reapIdleSessions 每 30s 回收空闲>60s 的会话进程，disposed.add）后，再切换智能体（含 missing 会话点重选恢复）必失败一次——switchAgent 不走 ensureStarted、未清除 disposed 标记，_createSession 建好新进程后末尾命中 disposed.has() 把刚建好的进程拆掉并抛「会话已清理」（HTTP 400），前端只弹「切换失败」toast，第二次重选才能成功。missing 会话更易触发：发消息被 agent_missing 拦截（不走 ensureStarted、不清 disposed），点开查看不刷新活跃时间，回收后残留标记无人清理。
- 修复：switchAgent 在拆除+重建的连续同步段内补 `disposed.delete(sessionId)`，与 ensureStarted 语义对齐（被 dispose 过的 sessionId 允许重建）；一次重选即成功。
- 验证：agent-manager.test 新增「会话已被 dispose（空闲回收）后切换不抛『会话已清理』，能重建」用例（修复前红灯复现同款报错，修复后绿灯：新 handle 存活 + 持久层已更新）；agent-manager 全文件 118 pass；kernel 权威 gate（bun run test --isolate）全部通过。
- 影响范围：packages/kernel/src/agent-manager.ts（switchAgent）、tests/agent-manager.test.ts。

## 2026-09-02 — fix(frontend): 切换智能体失败静默（未捕获 rejection + 无提示）

- 问题：顶部切换智能体（AgentSwitcher）或 agent_missing 重选恢复（AgentMissingModal）时，set-agent 请求失败（如目标会话已被空闲回收、kernel 回「会话已清理」400）会变成未捕获的 Promise rejection，console 报 `Uncaught ApiError`，UI 无任何提示——用户点切换后确认框默默关闭、角色未变，无从知晓失败。
- 修复：两处 `void api.post(.../set-agent...)` 均补 `.catch()`，失败弹出 error toast（agentSwitcher.switchFailed：「切换智能体失败，请重试」），不再静默/不打印未捕获异常。
- 验证：AgentSwitcher.test 新增「set-agent 失败（会话已清理）时弹出错误 toast」用例，注入 /set-agent 400 reject，断言 error toast 弹出 + 确认框关闭；frontend 全量单测 2078 pass。
- 影响范围：packages/frontend/src/components/AgentSwitcher.tsx、AgentMissingModal.tsx、src/i18n/locales/zh.ts、en.ts、tests/AgentSwitcher.test.tsx。

## 2026-09-02 — fix(frontend): 会话顶部角色删除后提示可点击重选

- 问题：改造为只读角色展示后，会话主角色被删除时顶部显示「原智能体已删除，点击重选」，但只读逻辑把 missing 态也做成了静态不可点，点击无反应——缺失恢复入口失效，会话无法重选角色。
- 修复：只读模式仅约束「角色存在时不能随意切换」；角色被删除（missing）属异常态，回落交互式 AgentDropdown——点击警示提示可展开列表重选、确认后发 set-agent（缺失恢复语义不受只读约束）。
- 验证：AgentSwitcher.test 新增「只读模式 + 角色已删除：点击提示仍可展开重选列表并恢复」用例，行为级锁定（点击 switcher-missing → 展开 → 选 dev → 确认 → 发 set-agent）；frontend 全量单测 2077 pass。
- 影响范围：packages/frontend/src/components/AgentSwitcher.tsx、tests/AgentSwitcher.test.tsx。

## 2026-09-02 — fix(frontend): 会话标题撑高顶部 + 角色入口改只读展示

- 问题①：会话标题（session.title）无行数限制，窗口太窄时文字折行无限撑高顶部 header。修复：标题 span 加 `leading-[1.5] line-clamp-2`，最多折叠两行、超出省略；沿用 AgentGalleryModal 既有先例。
- 问题②：会话顶部切换角色入口（AgentSwitcher）在会话进行中可点开下拉换角色。按需求改为__只读展示当前角色__（emoji 色块+角色名），不再渲染下拉、点击不弹菜单、不发 set-agent 请求（接口不动，其余 AgentDropdown 入口——新建会话/设置/自动化——保留）。
  - AgentSwitcher 组件新增 readOnly 开关；session:updated 广播仍更新 primaryAgent 显示并追加「已切换为」分隔行（缺失恢复逻辑保留）。
  - 布局：角色显示移入标题下的状态行（与项目目录、会话状态并排）且靠右；去掉 pill 边框/背景/圆角盒子感，改为与状态行协调的简洁文本标签。
- 验证：单测（SessionView.test + AgentSwitcher.test，含新增只读断言）55 pass；e2e 改后链路（建 A2 会话 → 删 A2 → 缺失恢复）通过；900×650 小窗口截图实证头部布局。
- 影响范围：packages/frontend/src/components/SessionView.tsx、AgentSwitcher.tsx、tests/SessionView.test.tsx、tests/AgentSwitcher.test.tsx、e2e/agents.spec.ts（删「5 会话中切换」，改「4」建 A2 会话）、顺带清理死 import existsSync。

## 2026-09-02 — fix(frontend): 小窗口下半屏预览关闭按钮被裁不可见

- 问题：主窗口缩小时，split 半屏 HTML 预览工具栏（单行 nowrap + 地址栏 shrink-0 锁宽）需求总宽超出面板宽，flex 行向右溢出后被 split 容器 overflow-hidden 裁剪，行尾的关闭按钮最先不可见、无法点击。
- 修复：工具栏加 flex-wrap 兜底（极窄面板下按钮行整体换行）；地址栏容器 shrink-0 → min-w-0 可收缩（中窄面板下地址栏先让位到 160px 下限，按钮保持可见）。大窗口布局不变。
- 验证：新增组件契约测试（flex-wrap + min-w-0）与 E2E（900×650 视口下关闭按钮完整落在视口内且可点，修复前右缘溢出至 1047px）；frontend 全量组件测试 2073 pass。
- 顺带：e2e/helpers createProject 容错补 duplicateCwd（kernel REST 化后错误形态变英文 code，旧中文「已存在」判定永不命中，serial 用例从第 2 个起必挂——该缺口同时挡住了 browser-preview 等用例的回归路径）；两处 bare object 参数类型收紧。
- 影响范围：packages/frontend/src/components/BrowserPanel.tsx、e2e/helpers.ts、新增 e2e/preview-close-narrow.spec.ts 与组件契约测试。
...
- 版本：0.3.9 → 0.3.10。
- 主要：修复浮动预览首次打开不居中的问题；修复快捷键切换元素选中时开了又关的问题。
- 验证：typecheck 全绿；四层回归全绿（隔离 worktree）。
- 影响范围：frontend（FloatPreview/browser）、kernel（preview-inspect.js）。
...
- 影响范围：packages/kernel/src/assets/preview-inspect.js、packages/frontend/src/{components/BrowserPanel.tsx,components/blocks/HtmlPreview.tsx}、packages/frontend/src/components/**tests**/browser-inspect-toggle.test.tsx。
...
- 版本：0.3.4 → 0.3.5。
- 主要：预览对未识别后缀做文本嗅探兜底（只要是文本即可预览），预览大小上限放宽到 5MB；引导消息携带的图片/文件在消息里以 chip 卡片展示；修复多插件并行升级互踩（此前只有一个成功、其余回退）。
- 验证：typecheck 全绿；四层回归全绿。
- 影响范围：kernel（preview/插件升级队列/路由与集成测试）、frontend（引导附件卡片）。
...
- 版本：0.3.2 → 0.3.3。
- 主要：青蛙主题官网（中英双语）上线并在「关于」页直达；长回复折叠时自动滚动收敛优化，不再来回跳动；新建会话文件树右键菜单超窗不可点问题修正。
- 验证：typecheck 全绿；四层回归全绿。
- 影响范围：website/（新增双语官网）、scripts/publish-web.ts、frontend（AboutSection/events/MessageList）、e2e。
...
- desktop 测试跨平台修复：kernel-updater.integration.test 的打包步骤原用 Unix-only 的 zip 命令（Windows 无 zip CLI，spawnSync status=undefined 必挂，且 HAS_ZIP 守卫导致整个测试文件不注册）——改平台分支：Windows 用内置 tar -a（bsdtar 按扩展名出 zip，实测文件头 PK 50 4B 03 04 验证）打包，macOS/Linux 保持 zip；HAS_ZIP 守卫同步按平台检测（Windows 检测 tar --version）。
- 验证：TDD 红→绿；agent-manager.test 新增 2 用例（clearQueue 调 RPC + 双队列清空 + queue_update 推送；RPC 失败兑底仅清本地）；steer-queue-poc.test 的 clearFollowUpList 调用同步迁移；runtime-deps.test 新增正向用例（动态 kernel + seed 依赖变化 → 覆盖清单 + 删标记 + KERNEL_BIN 保留）+ 守卫用例数据键名修正（deps → dependencies 贴近真实结构）8/8 绿；kernel 相关 161 pass（唯一失败「bun:image 压缩 webp」系 Windows ftruncate EPERM 环境问题，基线即失败与升级无关）；desktop 全量 221 pass / 0 fail（历史首次全绿：kernel-updater.integration 在 Windows 跑通完整链路 下载→sha 校验→tar 解压→覆盖 runtime→清理）；前端全量 2052 pass / 0 fail；双包 typecheck 绿。
...
- 影响范围：packages/kernel/package.json + bun.lock、src/{rpc-client,agent-manager,ws-server}.ts、tests/{agent-manager.test,steer-queue-poc.test,fixtures/fake-session-client}、packages/desktop/src/util/runtime-deps.cjs、tests/runtime-deps.test.ts、src/util/kernel-updater.integration.test.ts、frontend SessionView.tsx、CHANGELOG。
...
- 实现：①tokens.ts 新增 expandedTextToHtml(text, {knownSkills, hideTrigger})：先把展开形态还原回 token（/skill:x → $[x]，knownSkills 过滤防任意文本误判，约束同 MessageList.formatSkillBlocks），再走 textToHtml（全量 escapeHtml 转义）；②SessionView 队列面板两处改用它渲染（dangerouslySetInnerHTML + pi-lens-ignore: dangerously-set-inner-html，同 MessageList 历史消息模式），knownSkills 沿用稳定数组 selector + useMemo 模式；③ensureChipStyles 注入 chip 样式。同日两次迭代（用户实测反馈「选中文件夹没有渲染出来」「文件夹也有数字」）：初版 file 还原用「含 / 或含 .」启发式漏掉无扩展名一级文件夹名（#docs），中版改任意 #非空白+排除纯数字又与「文件夹名可以是纯数字（如 #path:2024）」冲突——最终方案：expandTokens 的 file 展开统一带 path: 锚（#[x]→#path:x，拖拽的 #[path:/abs] 已带前缀不重复），还原只认 #path: 锚，零误判且任意文件夹名（含纯数字）全覆盖；裸 #词（#docs、#1、# 标题）不猜。
- 已知边界：命令 chip 展开形态 /cmd 与任意斜杠文本无法区分，不还原（保持原样）；历史消息里 file 同样只显示纯文本（既有基线，未在本变更内扩大范围）。
- 验证：tokens.test 新增/更新 9 用例（skill 还原/knownSkills 过滤/#path: 锚含纯数字文件夹/裸 #词不猜/原样 token 不重复还原/hideTrigger/element/转义防注入）40/40 绿；SessionView.test 新增 2 组件用例（队列渲染 .chip-skill/.chip-file 断言 data-token 完整路径含 path: 前缀 + 未命中技能不渲染）42/42 绿；Composer.test 的 steer 用例同步上轮附件修复断言（/steer body 含 attachments + 发送后清空）24/24 绿；MessageList/ComposerTextarea/store-session 回归全绿；前端正式姿势（--isolate --timeout=20000）全量 1501 tests / 0 fail；四包 typecheck 绿。过程中修复：新 hooks 误置于 early return 之后触发 React #300（同 0822 历史坑，已移至 early return 前）。
- 影响范围：packages/frontend/src/{quick-invoke/tokens.ts,components/SessionView.tsx}、tests/{tokens,SessionView}.test.tsx。

## 2026-09-01 — fix(kernel/test): 裸 bun test 全量挂死根治（任意机器可直接跑全量）

- 背景：kernel 全量测试必须走 bun run test（scripts/test.ts 带 --isolate+限流+preload+integration 补跑），直接裸 bun test 会永久挂死（实验实测 900s+ 无输出），而缺少防护知识的人/机器一跑就中招。根因（系统化调试 + 采样实证）：① fs-open-env.test.ts 的 mock.module("node:child_process") 在无 --isolate 时泄漏到同 worker 后续文件（Bun 1.4 mock.module 返回 undefined、无恢复 API）；② 后续 preview-inspect.integration 启动完整 kernel 时 applySystemProxy→readMacSystemProxy 的 execFile("scutil") 拿到永不回调的 stub → 顶层 await 永挂 → runner 等文件永不完成；③ 伴生：无 preload 时 WA_PI_DIR 不隔离（kernel 以真实 ~/.pi/agent 启动与宿主竞争）、代理 env 不清。最小复现：无 isolate 混跑 fs-open-env + preview-inspect 两文件即挂死，加 --isolate 即 1.5s 过。
- 修复（三层，拆掉挂死链每一环）：① settings-store 的 readMacSystemProxy/readRegValue 加 5s 兑底超时（guard setTimeout——execFile 自身 timeout 只对真实子进程生效，对 mock/异常环境的永不回调无效；对生产也有价值：scutil 真机挂起不再卡死 kernel 启动）；② kernel bunfig.toml 新增 [test].preload = tests/setup.ts——隔离与清代理对裸 bun test 全局生效，不再依赖显式传参；scripts/test.ts 移除显式 PRELOAD 传参（避免双份 mkdtemp），注释指向 bunfig；③ preview-inspect/preview-route 两个 integration 的 projects.json 改写到 shared 常量固化的 WA_PI_DIR（bun test 模式下 preload 目录固化，自设 env 无效——原写 TMP_ROOT 致混跑时 allowlist 放行失败 7 例），afterAll 删除还原；fs-open-env 头注释记录 mock 泄漏已知后果与兑底。
- 已知边界：裸全量（无 --isolate）仍会有 mock 跨文件互染的部分用例失败（实测 106 个，集中在 routes-chat/composer-attachments 等，单独跑均绿）——能跑完、失败可见，不再挂死；正式入口无此问题。bun <file>.test.ts 直接执行（非 runner 模式）时 kernel 常驻句柄导致进程不退出，属 bun 固有行为，运行测试请用 bun test。
- 验证：原挂死最小复现命令修复后 1.95s 全绿；四个 integration 单独跑 14 pass/0 fail；裸 bun test 全量 219s 跑完 1538 tests（不再挂死）；正式入口 bun run test 全绿（1526 tests + integration 补跑 4/4，EXIT=0，0 fail，优于 0830 基线的 53 个环境性失败）；settings-proxy 25 例、四包 typecheck 全绿。
- 影响范围：packages/kernel/{bunfig.toml,scripts/test.ts,src/settings-store.ts,tests/{fs-open-env,preview-inspect.integration,preview-route.integration}.test.ts}。

## 2026-09-01 — fix(steer): Ctrl+Enter 发送引导消息附件全链路丢失

- 背景：agent 运行中且无活跃引导时 Ctrl+Enter 发引导消息走独立的 /steer 分支，输入框附件（图片/大文本 snippet/文件）全链路丢失，且发送后附件残留在输入框。根因：wa-pi 的 steering 链路（前端 POST → HTTP 路由 → ws 分发 → steerMessage → steerList → rpc-client steer()）从设计起只承载纯文本；而 pi 引擎 steer(message, images) RPC 原生支持多模态，能力存在但未接线。
- 实现：①shared SteerMessageEvent/SteerImmediateMessageEvent 加 attachments 字段；②前端 Composer handleSendSteer 请求体携带 attachments 并在发送后清空附件（与 doSend 一致）；③routes/chat.ts 两个 steer 路由透传；④ws-server 两 case 透传；⑤rpc-client steer(message, images?)；⑥agent-manager steerMessage 加 opts.attachments，经 buildPromptContent 统一转换（大文本片段内联、文件转 path: 引用、图片转多模态 images），steerList 从 string[] 扩为 {text, images?}[]，settled 兕底 drain 补传 entry.images；⑦排队消息提升为引导时 images 从 followUpList entry 继承（前端只有文本无法重传）；⑧queue_update 转发给前端的 steering 仍映射为 string[]，前端契约不变。顺带：shared ToolCall.arguments 类型 Record<string,any>→Record<string,unknown>（pi-lens blocker，四包 typecheck 验证无下游破坏）。
- 验证：新增 kernel tests/steer-attachments.test.ts 5 用例（busy 直投/已有引导降级排队/空闲直发/提升继承/queue_update 契约）全绿；routes-chat steer 用例补 attachments 透传断言；frontend SessionView.test 新增「运行中 Ctrl+Enter 发引导带附件并清空」用例（40/40 绿）；7 个相关测试文件 163 pass / 1 fail（基线既有崩溃日志用例）；shared/kernel/desktop/frontend 四包 typecheck 绿。注意：kernel 全量 bun test 在本机会卡死，验证时按改动交集跑相关测试文件。
- 影响范围：packages/shared/src/types.ts、packages/kernel/src/{rpc-client,agent-manager,routes/chat,ws-server}.ts、packages/frontend/src/components/Composer.tsx、packages/kernel/tests/{steer-attachments,routes-chat,fixtures/fake-session-client}、packages/frontend/tests/SessionView.test.tsx。
...
- 验证：node --check（splash-html/runtime-deps/main）全绿；splash-html.test 9 pass、tests/runtime-deps 8 pass、src/util/**tests**/runtime-deps 10 pass、node-runtime 15 pass、kernel-updater 20 pass、updater 15 pass。
...
- 版本：0.2.29 → 0.2.30。
- 主要：新建会话页文件树根行撑破输入框出屏 + 右键菜单超出窗口不可点两处修复；新增 explorer 右键菜单钳制、根行容器钳制与文件树溢出 E2E。
- 验证：typecheck 全绿；四层回归全绿。
- 影响范围：frontend（NewSessionPane/ExplorerPanel）、e2e。
