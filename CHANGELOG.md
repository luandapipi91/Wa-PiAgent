...
- v0.6.0 发版：升版 0.5.6 → 0.6.0（34 提交）。内容：桌宠大改造（多屏归属屏边界/菜单不越屏与不被截断、窗口固定 560×660 + 窗口内部缩放、置顶、菜单期间暂停自发动作、点菜单外直接生效、任务完成动作收敛为四个并从互动菜单移除）；定时任务/机器人模型改必选（移除「跟随默认」）+ 定时任务排除 ask_user_question；子智能体中断部分进度不丢（看门狗误杀双修）。验证：四包 typecheck + kernel 全量回归 + 双端打包 gate。
