...
## 2026-08-18 — feat(desktop): 桌面端日志文件 10MB 上限 + FIFO 裁剪

- 打包版 desktop.log 只增不减会一直膨胀；现超过 10MB 时丢弃最旧的行、只保留最新 8MB（按换行对齐，不留半截行），所有文件操作串行化避免裁剪与追加并发冲突。重启后首次写入会 stat 存量计入上限。
- 影响范围：`packages/desktop/src/util/log.cjs`（createLogger 新增可选 maxBytes/keepBytes 参数，默认 10MB/8MB，调用方 main.cjs 不变）、`packages/desktop/tests/log.cjs.test.ts`（新增）。
