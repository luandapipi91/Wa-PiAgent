...
- 版本号 0.1.15 → 0.1.16，用于验证 macOS electron-updater 自动更新全流程（检测→下载→MacUpdater 安装→重启）。
...
- **feat(desktop)·进程登记簿**：登记 / 三重校验 / TTL / 启动清扫 / 失效可观测；清扫连带 kernel 子孙链（覆盖 pi 子进程盲区）。
- **feat(desktop)·端口治理**：启动端口占用静默自愈；升级安装前优雅停 kernel 并等端口释放；before-quit 同步杀进程树。
- **feat·应用图标更新**：logo.svg 换版（126 绿底）+ 全尺寸 icon 重生成。

### 修复

- 工具调用前流式 text 定格未闭合 markdown 语法出现空白气泡。
- 用户手动上翻后不再被自动滚动拉回底部（AI 回复中同样生效）。
- 委派子智能体时聊天界面不自动滚动到底部。
- 选中其他项目会话时侧边栏高亮不跟随切换。
- 所有 switch 开关跟随主题颜色。
...
- 进程管理：kernel 进程登记簿 + 端口自愈 + 优雅停
- 前端：流式渲染、自动滚动、侧边栏高亮、主题开关
- 图标：logo.svg + icon.ico/icns/png + tray 图标
