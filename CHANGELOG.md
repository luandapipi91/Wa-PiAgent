...
- perf(shared+kernel): 委派提示词优化（含口径修正）——fleet 选择正确率 88%→100%（R7/R8/R10/R12/R14 五轮实测 16/16），委派相关提示词（DELEGATE + FLEET + 系统 delegate-mechanism 段 + delegate-roster 注入段）合计 592 tok ≤600：①判定「先查顺序词（先…再…/然后/按…结果/取决于）→ 一律逐个 delegate、禁止 fleet；否则数对象 ≥2 个互不依赖对象 → 必须一次 fleet 并行、禁止合成一个 delegate；单对象探索/审计/调查或改代码/写文件 → delegate；单点/需交互 → 不派」；②delegate-roster 紧凑化 183→89 tok（XML 块→一行一智能体，去定义文件路径）；③memory_search 例外收紧为「仅限提问，不含改代码/写文件任务」；④新增测量脚本 packages/kernel/scripts/eval-prompt-budget.ts（四部分合计 ≤600，超预算退出码 1，口径 CJK≈1/其余 4 字符≈1）；⑤迭代轨迹 R1-R15：75→88→88→81→94→94→100→100→94→100→94→100→94→100→94%（~/.wa-pi/eval-dt-iterations.md）；同文案下 100% 可达但连续 2 轮未成（R13/R15 各 1 条边界案例漏派，纯波动）——已按停止规则停下待用户裁定（放宽预算/放宽正确率/继续迭代）；测试 shared 17 + kernel 45 全绿
