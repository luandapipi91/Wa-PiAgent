...
- 小白指引：API Token 输入框旁新增「?」帮助入口，点击弹窗显示**图文指引**（SVG 界面示意图 + 数字步骤 + 关键链接），带 ✕ 关闭按钮。EdgeOne：Makers 控制台「设置 → API Token」Tab 创建（填描述 + 过期时间）；Cloudflare：API Tokens 页面 Create Token 模板。
