...
- 验证：四层回归全绿——kernel/frontend 单测与 typecheck、scripts/file-media-it.sh curl 集成（/file 媒体白名单 + Range 206）、e2e/chat-media-preview.spec.ts 1 passed（真实浏览器验证 2 列网格、内联视频、画廊切换、复制路径读回剪贴板、Esc 关闭；projects.json 写会话 + page.route 注入 assistant 消息，不依赖真实 LLM）。
