...
- perf(shared+kernel): 委派提示词优化（含口径修正）——fleet 选择正确率 88%→100%（R7/R8/R10/R12 多轮实测 16/16，波动区间 15-16/16），委派相关提示词（DELEGATE_DESCRIPTION + FLEET_DESCRIPTION + 系统 delegate-mechanism 段 + delegate-roster 注入段）合计 593 tok ≤600：①判定改「先查顺序词（先…再…/然后/按…结果/取决于）→ 一律逐个 delegate、禁止 fleet；否则数对象 ≥2 个互不依赖对象（两份文件/多个目录/两组并行）→ 必须一次 fleet 并行、禁止合成一个 delegate；单对象 → delegate」；②delegate-roster 注入段紧凑化 183→89 tok（XML 块改一行一智能体列表、去定义文件路径）；③memory_search 例外收紧为「仅限提问，不含改代码/写文件任务」；④新增测量脚本 packages/kernel/scripts/eval-prompt-budget.ts（四部分合计 ≤600 为准，超预算退出码 1）；⑤迭代轨迹 R1-R13：75→88→88→81→94→94→100→100→94→100→94→100→94%（~/.wa-pi/eval-dt-iterations.md）；边界用例（[12] 写入类漏派 / [16] 顺序类误派）在轮间轮换，连续 2 轮全对仍待稳定；测试 shared 17 + kernel 45 全绿
