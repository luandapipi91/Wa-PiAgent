...
- 新增(frontend)：记忆列表前端分页状态机——store 删除全量 memories/archived，改 pageEntries/pageHasMore/pageCounts（limit=50 恒下推、offset=已加载数、loadMore 防重+去重）；search 参数扩展 dateFrom/dateTo → since/until 本地时区毫秒（dateFromToSinceMs/dateToToUntilMs）；setMemories 广播降级为变更信号（重拉第一页+检索态同参数重跑）；新增 searchMore 去重追加（内核 FTS 候选池上限 50）；store 单测 20 例红→绿；MemoryPage.tsx/测试解构改造已随任务 8 完成落地。
- feat(memory): 记忆管理支持按更新时间日期范围筛选与滚动加载分页（memory:list:page 协议 + react-day-picker 日期控件，检索态同步支持时间窗）；补齐日期筛选与滚动哨兵 E2E，清理 PoC 临时产物。
