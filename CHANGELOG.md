...
- feat(skills): 技能来源新增项目级 `<cwd>/.pi/skills` 并按「项目 → 内置(`dataDir/skills`) → 扩展包」优先聚合——同名先到者胜、被遮蔽者保留在 allSkills 并标记 `shadowed`，`scan` 的 dirs 携带范围信息（`{type, projectId, projectName, name}`）；项目来源由 ProjectStore 项目列表派生（`collectProjectSkillSources` / `projectSkillsDirOf`，空 cwd 跳过），会话技能路径按同一顺序传入 pi（`--skill` 项目在前）；已废弃的 settings.json `userSkillDirs` 彻底忽略（旧值原样保留、不再读写）。前端：技能页新增「全部 / 全局 / 项目」范围筛选（`SkillScopeDropdown`）与「全局 skill / 项目 skill（项目名）/ Plugin skill（包名）」来源标签，`filterSkillsByScope` 按范围剔除或放行项目技能（同名时项目那条生效、被遮蔽项在全部范围隐藏），切换会话或项目时同步范围。移除技能目录手动增删的全部入口与接口（设置页目录区去掉增删按钮，`POST/DELETE /api/skills/dirs` 现返回 404）。修复技能开关 REST 载荷方向（9578175c：此前发的是旧状态，改为发送期望新状态 `enabled`）。测试改造：E2E 由写真实技能目录改为直接注入内置技能并覆盖项目级范围断言（收紧等待匹配维度到内置来源）。验证：kernel 聚焦 6 文件 143 pass / 0 fail、前端聚焦 3 文件 48 pass / 0 fail、四包 typecheck 全绿；接口层起隔离 kernel（独立 WA_PI_DIR + `WA_PI_WS_PORT=19776`，`bun run src/desktop-server.ts`）实测 `GET /api/skills` 含 project 目录项与 project 来源技能、同名时内置项 `shadowed=true`、`POST/DELETE /api/skills/dirs` 均 404、toggle 缺参 400、`enabled:false/true` 开关往返生效；E2E 隔离端口（19776 / 5181）跑 skills/channels/quick-invoke/agents 四 spec 24 pass / 0 fail；全量回归 `bun run test` 四包 0 fail（kernel 2111 主批 + 集成/负载敏感补跑批次全绿、shared 171、desktop 281、frontend 2799）。
