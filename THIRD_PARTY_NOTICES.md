...
wa-pi 遵循各依赖自身的开源许可证。所有直接运行时依赖均为宽松许可证（MIT / Apache-2.0 / ISC），无 copyleft 传染性依赖。
...
- **BlueOak-1.0.0**：glob、minimatch、path-scurry、yallist、lru-cache 等（经 @earendil-works/pi-coding-agent 链引入）。BlueOak 为宽松许可证，非 copyleft。
