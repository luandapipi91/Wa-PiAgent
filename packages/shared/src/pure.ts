...
export function makeAgentStateKey(
 projectId: string,
 agentName: AgentName,
): AgentStateKey {
...
export function parseAgentStateKey(key: AgentStateKey): {
 projectId: string;
 agentName: AgentName;
} {
...
/**
 * 「系统打开」出口的环境净化：剥离 WA_PI_* 前缀的 wa-pi 内部变量（端口/目录/运行时标记等），
 * 其余系统变量原样保留。
 *
 * 背景：kernel 把文件/目录交给系统默认应用（mac open / win start / linux xdg-open）时，
 * macOS 的 open 会把调用者环境经 LaunchServices 传给目标应用——.command 脚本由 Terminal
 * 继承执行。若不剥离，被打开脚本（如项目 start.command → bun run dev）读到继承的
 * WA_PI_WS_PORT/WA_PI_WEB_PORT 后启动即 killPort，抢占/杀死宿主 wa-pi 实例的端口；
 * 访达双击走 launchd 环境干净，故无此问题。
 *
 * 只剥 wa-pi 命名空间、不白名单化：LANG/SSH_AUTH_SOCK 等系统变量影响默认应用行为，
 * 白名单方式遗漏任一都会难以排查地破坏打开动作。
 */
export function sanitizeOpenEnv(
 env: Record<string, string | undefined>,
): Record<string, string> {
 const cleaned: Record<string, string> = {};
 for (const key of Object.keys(env)) {
  const value = env[key];
  // 逐键拷贝而非展开合并：Bun 下 { ...process.env } 有丢 key 的坑（见 kernel rpc-client）
  if (value !== undefined && !key.startsWith("WA_PI_")) cleaned[key] = value;
 }
 return cleaned;
}
