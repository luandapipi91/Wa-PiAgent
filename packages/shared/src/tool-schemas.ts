...
export const DELEGATE_DESCRIPTION = [
  ...
  "- 例外（仅限提问，不含改代码/写文件任务）：知识类/过程类提问（项目结构、依赖与方法清单、项目约定、历史决策、上一轮改动、构建/测试方式、环境事实、踩过的坑）→ 先 memory_search，再决定是否委派",
  "- 先查顺序词（先…再…/然后/按…结果/取决于）→ 一律逐个 delegate，禁止 fleet（即使多对象）",
  "- 否则数对象：≥2 个互不依赖的对象分别做同样的事（两份文件/多个目录/两组并行……）→ 必须一次 fleet 并行（每对象一个子任务），禁止合成一个 delegate；此后的「汇总/对比」由自己完成，不算依赖",
  "- 单对象探索/审计/调查 → delegate（哪怕一个文件）；单点查询/需交互 → 不派",
  "任务自含范围/输出/约束；返回值直接采用。",
].join("\n");
