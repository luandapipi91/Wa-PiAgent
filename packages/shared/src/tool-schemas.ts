...
export const DELEGATE_DESCRIPTION = [
  "隔离上下文中运行子智能体并返回结果。判定（默认委托）——首个调用即派发：",
  "- 例外（仅限提问，不含改代码/写文件任务）：知识类/过程类提问（项目结构、依赖、约定、历史决策、上一轮改动、构建/测试方式、环境事实、踩过的坑）→ 先 memory_search，再决定是否委派",
  "- 先查顺序词（先…再…/然后/按…结果/取决于）→ 一律逐个 delegate，禁止 fleet（即使多对象）",
  "- 否则数对象：≥2 个互不依赖的对象分别做同样的事（两份文件/多目录/两组并行……）→ 必须一次 fleet 并行（每对象一个子任务），禁止合成一个 delegate；「汇总/对比」自己做，不算依赖",
  "- 单对象探索/审计/调查，或需先读懂结构的改动/新增函数（含多文件改动）→ delegate（哪怕一个文件）；纯注释/文案类一行小改自己做；单点查询/需交互 → 不派",
  "任务自含范围/输出/约束。",
].join("\n");
