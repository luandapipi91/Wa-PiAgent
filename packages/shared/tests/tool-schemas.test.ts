// tool-schemas 共享模块测试
// 验证所有宿主工具的 description/schema 可从 @hiagent/shared 统一导入，
// 消除 bridge-extension.ts 与 delegate-tool/ask-tool/amaster-memory 之间的文案重复。

import { test, expect } from "bun:test";

test("DELEGATE_DESCRIPTION 可从 @hiagent/shared 导入且内容非空", async () => {
  // 当前 tool-schemas.ts 还不存在 —— 预期 import 失败，测试红灯
  const { DELEGATE_DESCRIPTION } = await import("@hiagent/shared/tool-schemas");
...
test("FLEET_DESCRIPTION 可从 @hiagent/shared 导入", async () => {
  const { FLEET_DESCRIPTION } = await import("@hiagent/shared/tool-schemas");
  expect(typeof FLEET_DESCRIPTION).toBe("string");
  expect(FLEET_DESCRIPTION).toContain("parallel");
});

test("ASK_DESCRIPTION / ASK_PROMPT_GUIDELINES 可从 @hiagent/shared 导入", async () => {
  const { ASK_DESCRIPTION, ASK_PROMPT_GUIDELINES } = await import("@hiagent/shared/tool-schemas");
...
test("memory 工具描述可从 @hiagent/shared 导入", async () => {
  const {
    MEM_TARGET_DESC,
    MEM_SCOPE_DESC,
    MEM_ADD_DESC,
    ...
    MEM_READ_DESC,
    MEM_READ_SNIPPET,
  } = await import("@hiagent/shared/tool-schemas");
