...
test("DEFAULT_AGENT_TOOLS 含 Pi 内置文件工具、网络工具与 amaster memory 记忆工具", () => {
	...
	// pi-web-access 网络工具族：来源核查
	expect(DEFAULT_AGENT_TOOLS).toContain("source_check");
...
test("DEFAULT_AGENT_TOOLS 不再包含已移除的 pi-hermes-memory 遗留工具 session_search", () => {
	expect(DEFAULT_AGENT_TOOLS).not.toContain("session_search");
});
