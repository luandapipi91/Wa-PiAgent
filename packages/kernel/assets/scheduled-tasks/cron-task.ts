// wa-pi-cron-task-asset v1
/**
 * wa-pi 定时任务 CLI（由 wa-pi kernel 自动分发，请勿手工编辑——旧版会被自动覆盖升级）。
 *
 ...
 *   add --name N --agent A --schedule '<json>' [--model M] [--disabled] --prompt P
 *   set <id> <key> <value>        修改字段（key: name/enabled/time/agent/model/prompt）
 *   validate <id>                 校验任务文件
 *   test <id>                     校验 + 显示未来 5 次触发时间（不执行）
 *   run <id>                      立即触发执行（需 wa-pi kernel 在线）
 *
 * 任务文件就在本目录 tasks/ 下，可直接阅读/编辑；改动会被 wa-pi 自动热加载。
 */
import { readFileSync, writeFileSync, readdirSync, existsSync, mkdirSync, renameSync, statSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";

const BASE_DIR = dirname(fileURLToPath(import.meta.url)); // .wa-pi/scheduled-tasks
...
function parseTask(content: string, id: string, file: string): any {
	...
	return { id, file, ...data, enabled: data.enabled ?? true, prompt };
}

function toCron(schedule: any): string {
	const [h, m] = (schedule.time ?? "00:00").split(":").map(Number);
	switch (schedule.type) {
		case "minute": { const n = schedule.intervalMinutes ?? 1; return `${n === 1 ? "*" : `*/${n}`} * * * *`; }
		case "hourly": {
		...
		default: throw new Error(`未知 schedule.type: ${schedule.type}`);
	}
}
...
function parseField(field: string, min: number, max: number): Set<number> {
	...
	for (const part of field.split(",")) {
		...
		if (base === "*") { lo = min; hi = max; }
			...
			else if (/^\d+$/.test(base)) { lo = hi = Number(base); }
...
function kernelBaseUrl(): string {
	const waDir = process.env.WA_PI_DIR || join(process.env.HOME || ".", ".pi", "agent");
	...
	try { process.kill(info.pid, 0); } catch { throw new Error("wa-pi kernel 不在线（pid 已失效），请先启动 wa-pi"); }
...
function main(): void {
	...
	switch (cmd) {
		case "help":
		case undefined:
			console.log(readFileSync(fileURLToPath(import.meta.url), "utf8").match(/\/\*\*([\s\S]*?)\*\//)?.[1]?.trim());
			return;
		case "list": {
			const files = listTaskFiles();
			if (files.length === 0) { console.log("（暂无任务）"); return; }
			for (const { id, file } of files) {
				try {
					const t = parseTask(readFileSync(file, "utf8"), id, file);
					const cron = toCron(t.schedule);
					const next = nextRunTimes(cron, 1)[0];
					console.log(`${t.enabled ? "●" : "○"} ${id}\t${t.name}\t${cron}${next ? `\t下次: ${fmtTime(next)}` : ""}`);
				} catch (err) {
					console.log(`✗ ${id}\t配置错误: ${err instanceof Error ? err.message : err}`);
				}
			}
			return;
		}
		...
		case "add": {
			...
			if (!opts.schedule) fail("缺少 --schedule（JSON，如 {\"type\":\"daily\",\"time\":\"09:30\"}）");
			if (!opts.prompt) fail("缺少 --prompt");
			let schedule: any;
			try { schedule = JSON.parse(opts.schedule); } catch { fail("--schedule 不是合法 JSON"); }
			const data = {
				name: opts.name, schedule, agentId: opts.agent,
				model: opts.model ?? undefined, enabled: !args.includes("--disabled"),
			};
			const content = serializeTask(data, opts.prompt);
			parseTask(content, "check", "check"); // 写前校验
			const base = sanitizeTaskId(opts.name);
			let id = base;
			for (let i = 2; existsSync(join(TASKS_DIR, `${id}.md`)); i++) id = `${base}-${i}`;
		...
		case "set": {
			...
			if (!id || !key || !value) fail("用法: set <id> <key> <value>（key: name/enabled/time/agent/model/prompt）");
			const t = loadTask(id);
...
try { main(); } catch (err) { fail(err instanceof Error ? err.message : String(err)); }
