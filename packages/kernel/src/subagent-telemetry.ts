...
// 落盘位置：~/.hiagent/subagent-telemetry.jsonl
...
export function computeSpawnTelemetry(input: SpawnTelemetryInput): SpawnTelemetryRecord {
  ...
  return {
    ...
    compressionRatio: childOutputTokens > 0 ? returnTokensEst / childOutputTokens : 1,
    hasOutput: input.returnText.trim().length > 0,
  };
}

export function summarizeSpawnTelemetry(records: readonly SpawnTelemetryRecord[]): SpawnSessionSummary {
  ...
  return {
    ...
    aggregateCompressionRatio:
      totalChildOutputTokens > 0 ? totalReturnTokensEst / totalChildOutputTokens : 1,
    totalCost,
  };
}
