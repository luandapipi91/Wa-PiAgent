...
export const registerProviderRoutes: RouteRegistrar = (
  r,
  callApi,
  ctx: RouteContext,
) => {
  r.add("GET", "/api/providers", async () =>
    callApi({ type: "provider:list" }),
  );
  ...
  r.add("GET", "/api/models/presets", async () =>
    callApi({ type: "model:presets" }),
  );
};
