...
const runtimeRequire = createRequire(
 join(WA_PI_DIR, "runtime", "package.json"),
);
 ...
 */
export function resolveExtensionEntryFile(
 pkgName: string,
 req = require,
): string {
 ...
 const pkg = effectiveReq(`${pkgName}/package.json`) as {
...
function readPiExtensionsDeclaration(pkgName: string): string[] | undefined {
 const parse = (req: NodeRequire) => {
  const pkg = req(`${pkgName}/package.json`) as {
   pi?: { extensions?: string[] };
  };
  const exts = pkg?.pi?.extensions;
  return Array.isArray(exts) && exts.length > 0 ? exts : undefined;
 };
 // 解析失败（包不在 repo node_modules）时回退 runtimeRequire 再试
 try {
  return parse(require);
 } catch {
  /* 回退 runtimeRequire */
 }
 try {
  return parse(runtimeRequire);
 } catch {
  /* 两级都失败，视为非 Pi 扩展 */
 }
 return undefined;
...
export function buildAdditionalExtensionPaths(): string[] {
 ...
 // provider-extension / wa-pi-bridge / wa-pi-tui-host 由 main()/ws-server 动态生成，
 // 首启或测试前可能尚未存在
