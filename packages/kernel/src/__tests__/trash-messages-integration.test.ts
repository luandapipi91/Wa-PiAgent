...
async function setupRouter(store: ProjectStore) {
	const { registerProjectSessionRoutes } = await import(
		"../routes/projects-sessions"
	);
...
async function createSessionWithMessages(store: ProjectStore) {
	...
	const lines = [
		JSON.stringify({
			...
			message: { role: "user", content: "你好", timestamp: Date.now() },
		}),
		JSON.stringify({
			id: "entry-2",
			type: "message",
	...
	parsed.sessions.find((x: any) => x.id === session.id).piSessionFile =
		filePath;
...
describe("Trash messages endpoint", () => {
	...
	test("GET /api/trash/sessions/:id/messages returns messages for deleted session", async () => {
		...
		const req = new Request(
			`http://localhost/api/trash/sessions/${session.id}/messages`,
			{
				method: "GET",
			},
		);
	...
	test("GET /api/trash/sessions/:id/messages returns empty for non-existent session", async () => {
		...
		const req = new Request(
			"http://localhost/api/trash/sessions/nonexistent-id/messages",
			{
				method: "GET",
			},
		);
