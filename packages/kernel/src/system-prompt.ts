...
export interface SystemPromptContext {
	...
	/** 项目级技能目录（`<project.cwd>/.pi/skills`）；无项目上下文时为空 → project 元素不出现 */
...
/** env-constraints 段的固定文案后缀（skill_directories 块之后拼接；以 \n 开头） */
...
export const DEFAULT_MEMORY_POLICY_PROMPT =
	...
	"检索范围默认为**全局 + 当前项目**（不跨项目）；要只看某一侧可显式传 scope。" +
...
export const COMPACT_MEMORY_POLICY_PROMPT =
	"## Memory Policy\n\n" +
	"先查再答（在委派或读代码之前）：问「这个项目是什么 / 有哪些 / 怎么跑」这类知识类、过程类问题（结构/依赖/接口与方法清单/约定/历史决策/上一轮改动/构建测试/环境事实/踩过的坑），第一个调用就是 memory_search，查完再决定是否委派；单点定义查询（答案一行念完）不必查。检索默认限「全局 + 当前项目」，不跨项目。L2/L3 不注入，不查等于不存在。\n" +
...
export const DEFAULT_PROMPT_SEGMENTS: PromptSegment[] = [
	...
	{ id: "env-constraints" }, // 动态：<skill_directories> 块（builtin + 可选 project）+ ENV_CONSTRAINTS_SUFFIX
...
function renderSegment(seg: PromptSegment, ctx: SystemPromptContext): string {
	...
	switch (seg.id) {
		...
		case "env-constraints": {
			const lines = [
				"<skill_directories>",
				`  <builtin>${ctx.builtinSkillsDir}</builtin>`,
			];
			// project 元素：仅在有项目上下文时出现（无项目 → 块内只含 builtin）
			if (ctx.projectSkillsDir) {
				lines.push(`  <project>${ctx.projectSkillsDir}</project>`);
			}
			lines.push("</skill_directories>");
