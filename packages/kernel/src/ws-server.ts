...
/** 预览上限：5MB（与 routes/fs.ts 的 checkPreviewable 保持同步） */
const MAX_PREVIEW_BYTES = 5 * 1024 * 1024;

/** 二进制喷探字节数：头部 8KB 含 NUL 字节判为二进制（Git 同款策略，与 routes/fs.ts 同步） */
const BINARY_SNIFF_BYTES = 8000;

/** 文本喷探：头部含 NUL 字节 → 二进制。用于 mime 未识别文件的纯文本兑底判定 */
...
> {
	...
	const isText =
		...
		mime === "image/svg+xml";
	// 放行图片预览：前端 FileViewer 拿到 base64 拼 data URI 展示（与 routes/fs.ts 对齐）
	const isImage = mime.startsWith("image/");
	if (!isText && !isImage) {
		// 兑底纯文本策略：扩展名未识别/未映射的文件，只要内容是文本就按纯文本预览；
		// 头部含 NUL 的真二进制才拒绝（与 routes/fs.ts 的 checkPreviewable 保持同步）
	...
	private async handle(
		...
		switch (event.type) {
			...
			case "contacts:sync-wecom": {
				...
				try {
					const { added, updated } =
						await this.opts.channelManager.syncWecomContacts(
							event.channelId,
							event.keywords,
						);
					this.broadcast({ type: "contacts:changed" });
					reply({ type: "contacts:sync-wecom-result", added, updated });
				} catch (err) {
					reply({
						type: "error",
						message: (err as Error).message,
