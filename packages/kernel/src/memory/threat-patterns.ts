...
const RAW_PATTERNS: readonly Rule[] = [
	...
	[
		"you\\s+are\\s+(?:\\w+\\s+)*now\\s+(?:a|an|the)\\s+",
		"role_hijack",
		"context",
	],
	[
	...
	[
		"never\\s+(?:\\w+\\s+)*(?:create|write)\\s+(?:\\w+\\s+)*(?:script|file)\\s+(?:\\w+\\s+)*disk",
		"anti_forensic_disk",
		"context",
	],
	[
		"unset\\s+\\w*(?:CLAUDE|CODEX|HERMES|AGENT|OPENAI|ANTHROPIC|PI)\\w*",
		"env_var_unset_agent",
		"context",
	],
	[
		"\\b(?:praxis|cobalt\\s*strike|sliver|havoc|mythic|metasploit|brainworm)\\b",
		"known_c2_framework",
		"context",
	],
	[
