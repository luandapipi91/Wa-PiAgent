...
//
// 内置 subagent 的 systemPrompt 定义在 builtin-agents.ts 的
// BUILTIN_AGENT_CONTENT 中（同时写入 ~/.pi/agent/agents/*.md）。不从任何包内部源码 import。
// 元信息（emoji/gradient/displayName）仍在 SUBAGENT_TYPES 常量。

import { SUBAGENT_TYPES } from "@wa-pi/shared";
import type { SubagentInfo, SubagentOverride, DelegationHints } from "@wa-pi/shared";
 ...
 * 内置 .md 用 pi-open-agents frontmatter 格式（无 displayName 等 WaPi 字段），
...
/**
 * 读取内置 subagent 的 systemPrompt：优先用户覆盖文件（~/.pi/agent/agents/<name>.md，
 * seedBuiltinAgents 写入后不覆盖用户编辑），文件不存在/为空时回退 BUILTIN_AGENT_CONTENT。
 * 供 agent-manager 构造子代理 spawn 配置（wa-pi 自实现的 .md 文件读取）。
 */
