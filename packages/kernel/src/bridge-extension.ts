// bridge-extension.ts —— 部署 hiagent-bridge 扩展到 GENERATED_DIR。
//
// RPC 模式下 pi 以子进程运行，SDK 的 customTools 机制不存在。
// 替代方案：kernel 把静态扩展文件 hiagent-bridge.extension.ts 连同依赖的
// tool-schemas.ts（来自 @hiagent/shared）复制到 GENERATED_DIR，
