// mcp-connector.ts — MCP 连接测试 / 工具列举
//
...
export async function resolveMcpDirectToolNames(
  ...
  const mode = (
    modeRaw === "none" || modeRaw === "short" ? modeRaw : "server"
  ) as "server" | "none" | "short";
  ...
  const results = await Promise.allSettled(
    enabled.map(async (server) => {
      const dt =
        ((server as McpServerConfig & { directTools?: unknown }).directTools as
          | true
          | string[]
          | undefined) ?? globalDirect;
...
function isJsonRpcSchemaError(err: unknown): boolean {
  ...
  return (
    err.message.includes("jsonrpc") && err.message.includes("invalid_union")
  );
}
