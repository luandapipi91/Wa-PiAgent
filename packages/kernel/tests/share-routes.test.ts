...
function mockEdgeOne(presetDomain = "wapi-abc.edgeone.run") {
    globalThis.fetch = (async (_url: any, init: any) => {
        ...
        const respond = (data: unknown) =>
            new Response(
                JSON.stringify({ Code: 0, Data: { Response: data } }),
                {
                    status: 200,
        ...
        switch (action) {
            case "DescribePagesProjects":
                return respond({
                    Projects: [
                        { ProjectId: "prj-1", PresetDomain: presetDomain },
                    ],
                });
            ...
            default:
                return new Response(
                    JSON.stringify({ Code: -1, Message: action }),
                    {
                        status: 200,
                    },
                );
...
test("5. delete → list 少一条且 pending+1；clear → list 空", async () => {
    ...
    const list1 = await (await router.handle(
        new Request("http://x/api/share/list"),
    ))!.json();
    ...
    const list2 = await (await router.handle(
        new Request("http://x/api/share/list"),
    ))!.json();
...
test("6. deploy → pending 归零", async () => {
    ...
    const before = await (await router.handle(
        new Request("http://x/api/share/list"),
    ))!.json();
    ...
    expect(typeof data.expiresAt).toBe("number");
    const after = await (await router.handle(
        new Request("http://x/api/share/list"),
    ))!.json();
...
test("7. refresh-link：存在 id 返回新 url；不存在 → 404", async () => {
    ...
    const missing = await post(router, "/api/share/refresh-link", {
        id: "nope",
    });
...
test("7b. refresh-link：本地有记录但从未成功部署 → 409", async () => {
    ...
    const res = await post(router, "/api/share/refresh-link", {
        id: "abcdef123456",
    });
...
test("部署失败广播 error 阶段（含错误信息）", async () => {
    ...
    createShareRoutes(
        router,
        {
            ...
            channel: "edgeone",
            cosFactory: () =>
                ({ putObject: (_o: any, cb: any) => cb(null) }) as any,
            settingsFile: join(dir, "settings.json"),
            pollIntervalMs: 1,
            broadcast: (e) => events.push(e),
        },
