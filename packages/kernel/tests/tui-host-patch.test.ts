...
function collect() {
	...
	return {
		sink,
		frames: () => lines.map((l) => JSON.parse(l) as Record<string, unknown>),
	};
}
...
describe("patchUiForTuiHost", () => {
	test("打上接管标记，且 custom 改为走 bridge", async () => {
		...
		await expect(ui.custom(factory, { overlay: true })).resolves.toBe(
			"panel-result",
		);
	...
	test("重复 patch 不叠加（同一个 bridge 则原样返回）", () => {
		const { ui } = makeUi();
		const asUi = ui as unknown as Record<string, unknown>;
		const bridge: TuiHostPatchBridge = {
			...fakeBridge,
			openCustom: async () => "first",
		};
...
describe("createPanelBridge", () => {
	...
	test("工厂同步返回空值：同样是 settle 为 undefined，不把 TypeError 抛回调用方", async () => {
		...
		const bridge = createPanelBridge({ sink });
		await expect(
			bridge.openCustom(() => undefined, undefined, undefined),
		).resolves.toBeUndefined();
		expect(frames().map((f) => f.type)).toEqual(["open", "close"]);
		expect(frames()[1]?.reason).toBe("cancel");
	});
	...
	test("同会话同时只开一个面板：第二个排队，首个结束后才 open", async () => {
		...
		await Bun.sleep(10);
		const second = bridge.openCustom(
			() => makeComponent("second").component,
			undefined,
			undefined,
		);
		await Bun.sleep(10);
	...
	test("openWidget：组件工厂走 widget 通道，采样帧带 widgetKey", async () => {
		...
		await waitFor(() =>
			frames().some((f) => f.type === "frame" && f.panelId === "w:dash"),
		);
		expect(frames().find((f) => f.type === "frame")?.lines).toEqual([
			"widget line",
		]);
		bridge.disposeAll();
	});
	...
	test("清除 widget（setWidget(key, undefined)）：发 close 并释放宿主与采样定时器", async () => {
		...
		await Bun.sleep(200);
		expect(
			frames().filter((f) => f.panelId === "w:dash" && f.type === "frame"),
		).toHaveLength(1);
	...
	test("openCustom 传 onHandle：overlay 降级路径也照常回调一次安全句柄", async () => {
		...
		bridge.handleInput({
			type: "cancel",
			panelId: frames()[0]?.panelId as string,
		});
		await expect(panel).resolves.toBeUndefined();
		expect(handles).toHaveLength(1);
	});
...
describe("patchUiForTuiHost + createPanelBridge：会话重建", () => {
	test("teardown 后新 bridge 接管同一个 ui 对象，custom 重新可用（reload 场景）", async () => {
		...
		const panel = ui.custom(
			() => makeComponent("after-reload").component,
			undefined,
			undefined,
		);
		...
		secondBridge.handleInput({
			type: "cancel",
			panelId: second.frames()[0]?.panelId as string,
		});
		await expect(panel).resolves.toBeUndefined();
