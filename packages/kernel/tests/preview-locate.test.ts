...
test("按 nth-of-type 定位兄弟元素", () => {
	expect(locateElement(HTML, "html > body:nth-of-type(1) > div#card > p:nth-of-type(2)")).toEqual({
		startLine: 7,
		endLine: 7,
	});
});
...
test("无 id 元素用 nth-of-type 段", () => {
	expect(locateElement(HTML, "html > body:nth-of-type(1) > div:nth-of-type(2) > span:nth-of-type(1)")).toEqual({
		startLine: 9,
		endLine: 9,
	});
});
...
test("selector 不匹配返回 null", () => {
	expect(locateElement(HTML, "html > body:nth-of-type(1) > ul:nth-of-type(1)")).toBeNull();
});
...
test("void 元素（img）自身起止同行", () => {
	const html = '<html>\n<body>\n<img src="a.png">\n</body>\n</html>';
	expect(locateElement(html, "html > body:nth-of-type(1) > img:nth-of-type(1)")).toEqual({
		startLine: 3,
		endLine: 3,
	});
});
