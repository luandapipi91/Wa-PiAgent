...
function cleanupPromptFile(sessionId: string) {
  try { rmSync(join(WA_PI_DIR, "tmp", "sysprompts", `${sessionId}.md`), { force: true }); } catch {}
}
...
test("delegate：handleTool 的 onProgress 被转发 + onSubagentProgress 携带正确 sessionId/toolCallId", async () => {
  ...
  const broadcasted: Array<{ sessionId: string; toolCallId: string; event: any }> = [];
  const fakes: FakeSessionClient[] = [];
  const am = new AgentManager({
    projectStore,
    configStore: { getAgent: mock(async () => devConfig) } as any,
    onEvent: () => {},
    onSubagentProgress: (sid, tcId, event) => broadcasted.push({ sessionId: sid, toolCallId: tcId, event }),
  ...
  await ctx!.handleTool(
    "delegate", toolCallId, { agent: "Plan", task: "设计个方案" },
    new AbortController().signal, handleOnProgress,
  );

  // 1. handleTool 的 onProgress 被转发（槽位透传成功）：收到一次 running 帧
  expect(forwardedToHandle.length).toBe(1);
  expect(forwardedToHandle[0]).toMatchObject({ status: "running", output: "mock 进度" });
  ...
  expect(broadcasted[0].event).toMatchObject({ status: "running", agent: "Plan" });
...
test("handleTool 未传 onProgress 时 onSubagentProgress 仍被触发（槽位为空走广播出口）", async () => {
  ...
  await ctx!.handleTool(
    "delegate", "tc-no-prog", { agent: "Plan", task: "设计" },
