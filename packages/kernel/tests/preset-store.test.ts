...
function tempAgentsDir() {
	const dir = join(
		import.meta.dir,
		".tmp-presets-" + Math.random().toString(36).slice(2),
	);
...
test("buildAgentConfigFromPreset 注入名字与预设字段", () => {
	...
	if (preset.color)
		expect(config.avatarColor).toBe(`${preset.color}-${preset.color}`);
	expect(config.systemPromptBody!.startsWith("你的名字是「林晓岚」。")).toBe(
		true,
	);
...
test("buildAgentConfigFromPreset 继承默认关系网（所有内置智能体）", () => {
	const preset = getPreset(listPresets()[0].id)!;
	const config = buildAgentConfigFromPreset(preset, "林晓岚");
	expect(config.partners.askTo).toEqual(ALL_AGENT_NAMES);
});
...
test("createAgentFromPreset 未知 id 返回 404", async () => {
	const dir = tempAgentsDir();
	const r = await createAgentFromPreset(
		new ConfigStore(dir),
		"not-exist-id",
		"林晓岚",
	);
