...
test("[第三层] session:messages 走 AgentSession.messages", async () => {
  ...
  const server = new WSServer({ configStore, projectStore, providerStore, skillManager, extensionManager: new ExtensionManager(join(projFile, ".."), { resolveEntryPath: () => "/fake/pi-lens/dist/index.js", readVersion: () => "0.0.0" }), agentManager, port: 0 });
  await server.start();
