...
test("readConfig: 正常解析", async () => {
  ...
  await writeFile(
    p,
    JSON.stringify({
      mcpServers: { s1: { command: "npx", args: ["-y", "test"] } },
    }),
  );
  const store = createStore() as any;
...
test("writeConfig: 写入后读取一致", async () => {
  ...
  await store.writeConfig(p, {
    mcpServers: { chrome: { command: "npx", args: ["-y", "chrome-mcp"] } },
  });
  ...
  await writeFile(
    ...
    JSON.stringify({
      mcpServers: {
        ...
        figma: { url: "http://localhost:3845/mcp" },
      },
    }),
  );
...
test("save: 改名 server (originalName)", async () => {
  ...
  await store.save(
    { name: "new-name", command: "echo", args: [] },
    undefined,
    "old-name",
  );
  const servers = await store.list();
...
test("save: 传 originalName 但服务器不存在抛错", async () => {
  ...
  await expect(
    store.save({ name: "test", command: "echo" }, undefined, "nope"),
  ).rejects.toThrow();
});
...
test("save: 传 originalName 与 config.name 相同应成功", async () => {
  ...
  await store.save(
    { name: "same-server", command: "echo", args: ["v2"] },
    undefined,
    "same-server",
  );
  const servers = await store.list();
