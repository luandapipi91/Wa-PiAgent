...
function DialogBody({ req, respond }: {
  req: ExtDialogRequest;
  respond: (fields: { value?: unknown; confirmed?: boolean; cancelled?: boolean }) => Promise<void>;
}) {
  ...
  return (
    <>
      <div className="p-4 border-b border-hairline">
        <div className="text-primary font-bold text-sm">{req.title ?? ""}</div>
      </div>
      <div className="p-4 text-sm text-secondary leading-relaxed flex flex-col gap-3">
        ...
        {req.method === "input" && (
          <input
            ...
            onKeyDown={(e) => { if (e.key === "Enter") void respond({ value: text }); }}
