...
export function RecentSessionsList({ onSelectSession }: Props) {
  ...
  const [listRef, setAnimateEnabled] = useAutoAnimate<HTMLDivElement>({ duration: ANIM_DURATION, easing: "ease-out" });
  useEffect(() => {
  ...
  return (
    <div ref={listRef} className="flex-1 overflow-y-auto overflow-x-hidden" data-testid="recent-sessions-list">
