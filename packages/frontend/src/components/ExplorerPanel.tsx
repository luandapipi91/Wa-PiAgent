...
function joinPath(parent: string, name: string): string {
  return parent.endsWith("/") || parent === "" ? parent + name : parent + "/" + name;
}
...
export function ExplorerPanel({
  ...
  const rebuildExpanded = useCallback(async (nodes: FlatNode[]): Promise<FlatNode[]> => {
    ...
    for (const node of nodes) {
      ...
      if (node.entry.isDir && expandedSet.has(node.entry.path)) {
        ...
        result[result.length - 1] = { ...result[result.length - 1]!, expanded: true, hasChildren: children.length > 0 };
  ...
  const toggleDir = useCallback(async (node: FlatNode) => {
    ...
    try {
      ...
      setFlatList(prev => {
        ...
        const next = [...prev];
        next[idx] = { ...currentNode, expanded: true, hasChildren: children.length > 0 };
  ...
  const startDrag = useCallback((e: React.PointerEvent, node: FlatNode) => {
    ...
    const fpath = node.entry.path, fname = node.entry.name;
    let dragging = false;
    let ghost: HTMLElement | null = null;

    const onMove = (ev: PointerEvent) => {
      if (!dragging && Math.abs(ev.clientX - e.clientX) < 5 && Math.abs(ev.clientY - e.clientY) < 5) return;
      if (!dragging) {
        dragging = true;
    ...
    const onUp = (upEv: PointerEvent) => {
      ...
      const ghostEl = ghost; ghost = null;
      ...
      const editor = target?.closest("textarea, [contenteditable], [role='textbox']");
      if (editor instanceof HTMLElement) {
        // 通过自定义事件通知 Composer 插入 @提及；Composer 监听后在编辑器光标处插入
        window.dispatchEvent(new CustomEvent("wa-pi:insert-mention", { detail: { text: `@${fpath} `, editor } }));
      }
    };
  ...
  return (
    <div className="ep-tree" style={{ overflow: "auto" }} data-testid="explorer-panel">
      {flatList.map(node => {
