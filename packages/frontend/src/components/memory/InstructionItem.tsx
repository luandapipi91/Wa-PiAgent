    ...
    <>
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} width={720} data-testid="instruction-view-modal">
          ...
          >
            <h3 className="text-[14px] font-extrabold text-primary m-0">{instruction.name}</h3>
            <button
              onClick={() => setOpen(false)}
              className="text-[12px] text-secondary px-2.5 py-1 rounded-md"
              style={{ border: "1px solid var(--hairline)", background: "transparent" }}
          ...
          <div className="px-5 py-2" style={{ borderBottom: "1px solid var(--hairline)" }}>
            <p className="text-[11px] text-tertiary font-mono m-0">{instruction.path}</p>
          </div>
          {/* 内容（可滚动，只读） */}
          <div
          ...
          >
            <pre
              className="text-[12px] text-secondary leading-relaxed whitespace-pre-wrap break-words m-0 font-mono"
