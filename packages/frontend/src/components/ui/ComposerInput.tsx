...
interface Props {
	...
	setAttachments: (
		value: AttachmentDraft[] | ((prev: AttachmentDraft[]) => AttachmentDraft[]),
	) => void;
	projectId?: string;
	sessionId: string;
	onSend: () => void;
...
export function ComposerInput({
				...
				onProgress: (matches) => {
					setFileResults(
						matches.map((m) => ({
							id: m.path,
							name: m.name,
							path: m.path.startsWith(projectCwd)
								? m.path.slice(projectCwd.length + 1)
								: m.path,
							isDir: m.isDir,
						})),
					);
	...
	const agentAskToEmpty = useMemo(() => {
		...
		const primaryConfig = allAgents.find(
			(a) => a.displayName === currentAgentName,
		);
	...
	const agentItems: MenuItem[] = useMemo(() => {
		...
		const candidates = allAgents.filter(
			(a) => askToSet.has(a.displayName) && a.displayName !== currentAgentName,
		);
		...
		(item: MenuItem) => {
			...
			if (triggerType === "command" && item.id.startsWith("pi:")) {
				...
				window.dispatchEvent(
					new CustomEvent("wa-pi:pi-command", {
						detail: { text: `/${cmdName}` },
					}),
				);
				return;
			...
			>
				...
				<div className="flex items-center justify-between px-3 py-2 border-t border-hairline">
					...
					<button
						...
						style={{
							background: canSend ? "var(--brand)" : "var(--hairline-strong)",
							color: "var(--on-brand)",
						}}
					>
						↑
