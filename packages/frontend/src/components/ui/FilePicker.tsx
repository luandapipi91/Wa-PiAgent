...
// 选择器展示顺序以此为准。
function sortEntries<T extends { name: string; isDir: boolean }>(
	entries: readonly T[],
): T[] {
...
async function walkToTarget(
	...
	for (const [id, item] of Object.entries(items)) {
		...
		if (
			isWin
				? lowerTarget.startsWith(rootPath.toLowerCase())
	...
	while (segIdx < allSegments.length) {
		const entries = sortEntries(
			(await listDir(currentPath, showHidden)).filter(
	...
	for (const id of keepIds) {
		...
		items[id] = {
			...
			children: item.children?.filter(
				(cid) => keepIds.has(cid) || isPlaceholderId(cid),
			),
		};
	}
...
function buildSearchTree(
	...
	const items: Record<TreeItemIndex, TreeItem<FsNodeData>> = {
		root: {
			...
			children: [],
			isFolder: true,
			data: { path: "", name: rootName, isDir: true },
		},
	};
...
export function FilePicker({
	...
	>({
		root: {
			...
			children: [],
			isFolder: true,
			data: { path: "", name: t("filePicker.loading"), isDir: true },
		},
	});
	const [searchTreeItems, setSearchTreeItems] = useState<Record<
		TreeItemIndex,
		TreeItem<FsNodeData>
	> | null>(null);
		...
		(async () => {
			...
			const items: Record<TreeItemIndex, TreeItem<FsNodeData>> = {
				root: {
					...
					isFolder: true,
					data: { path: "", name: t("filePicker.thisPc"), isDir: true },
				},
			};
			for (const [i, r] of roots.entries()) {
				items[`root_${i}`] = {
			...
			const targetPath =
				defaultPath && defaultPath.trim() ? defaultPath.trim() : home;
	...
	const handleMissingItems = useCallback(
		async (missingIds: TreeItemIndex[]) => {
			...
			for (const parentId of needLoad) {
				...
				if (!parent) continue;
				const entries = sortEntries(
					(await listDir(parent.data.path, showHiddenRef.current)).filter(
						(e) => showHiddenRef.current || !e.name.startsWith("."),
					),
				);
				...
				for (const [i, e] of entries.entries()) {
					...
					next[childId] = {
						...
						isFolder: e.isDir,
						data: {
							path: join(parent.data.path, e.name),
							name: e.name,
							isDir: e.isDir,
						},
					};
	...
	const handleExpandItem = useCallback((item: TreeItem<FsNodeData>) => {
		...
		void (async () => {
			const entries = (await listDir(path, showHiddenRef.current)).filter(
				(e) => showHiddenRef.current || !e.name.startsWith("."),
			);
	...
	const handleFocusItem = useCallback((item: TreeItem<FsNodeData>) => {
		...
		// 搜索过程中锁定搜索范围，同上
		if (searchQueryRef.current.trim()) return;
		if (item.data?.isDir && item.data.path)
			activeDirRef.current = item.data.path;
	}, []);
	...
	determineSearchRootsRef.current = () => {
		...
		for (const id of expandedItemsRef.current) {
			...
			if (
				it?.data?.isDir &&
				(!deepest || it.data.path.length > deepest.length)
			) {
				...
				<div className="bg-surface w-[680px] max-h-[85vh] rounded-lg flex flex-col border border-hairline shadow-lg">
					<div className="p-4 border-b border-hairline flex items-start justify-between gap-3">
						<div className="flex flex-col gap-1 min-w-0">
							...
							{currentSearchRoot && (
								<span
									className="text-[calc(11px*var(--font-scale))] text-tertiary max-w-[340px] truncate"
									data-testid="search-scope-hint"
									title={currentSearchRoot}
								>
						...
						<div className="flex flex-col items-end gap-1 shrink-0">
							...
							{isSearching && searchDuration !== null && (
								<span
									className="text-[calc(11px*var(--font-scale))] text-tertiary"
									data-testid="search-duration"
								>
					...
					<div
						className="flex-1 overflow-auto p-2 text-primary"
						style={{ minHeight: 320 }}
					>
						{isSearching && searchLoading && !hasSearchResults ? (
							...
							>
								<Tree
									treeId="file-picker"
									rootItem="root"
									treeLabel={t("filePicker.treeLabel")}
								/>
							</ControlledTreeEnvironment>
						)}
					</div>
