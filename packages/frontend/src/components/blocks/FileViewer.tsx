...
const IMAGE_EXTS = new Set([
  "png", "jpg", "jpeg", "gif", "webp", "svg", "bmp", "ico", "tiff", "tif", "avif", "apng",
]);
...
function guessLanguage(path: string): string {
  const map: Record<string, string> = {
    ...
    json: "json", rs: "rust", md: "markdown", css: "css", html: "html",
    py: "python", go: "go", java: "java", sh: "bash", yaml: "yaml", yml: "yaml",
    toml: "toml", sql: "sql", xml: "xml", svg: "markup", txt: "text",
  };
  ...
  return (
    <div className="flex flex-col h-full" data-testid="image-viewer">
      <div className="flex items-center gap-1 px-3 py-2 border-b border-hairline bg-surface">
        <span className="text-[12px] text-secondary flex-1 truncate">🖼️ {alt}</span>
        <button className="fv-btn" onClick={() => setZoom(z => clampZoom(z / 1.25))} title="缩小">➖</button>
        <span className="text-[11px] text-tertiary w-10 text-center">{Math.round(zoom * 100)}%</span>
        <button className="fv-btn" onClick={() => setZoom(z => clampZoom(z * 1.25))} title="放大">➕</button>
        <button className="fv-btn" onClick={onClose} title="关闭">✕</button>
      </div>
      <div
        ref={bodyRef}
        className="flex-1 overflow-hidden relative bg-canvas flex items-center justify-center"
...
export function FileViewer({ path, onClose }: FileViewerProps) {
  ...
  useEffect(() => {
    ...
    readFile(path)
      ...
      .catch((err: unknown) => {
        if (!alive) return;
        setError(`无法读取文件：${err instanceof Error ? err.message : "未知错误"}`);
        setLoading(false);
      });
    return () => { alive = false; };
  }, [path, image]);
  ...
  useEffect(() => {
    ...
    const onCopy = (e: ClipboardEvent) => {
      ...
      e.preventDefault();
      const ref = startLine === endLine ? `@${displayPath} :${startLine}` : `@${displayPath} :${startLine}-${endLine}`;
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full text-tertiary text-[13px]" data-testid="fv-loading">加载中…</div>;
  }

  if (unsupported) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3" data-testid="fv-unsupported">
        <span className="text-[32px]">📄</span>
        <span className="text-[13px] text-secondary">不支持预览该文件</span>
        <span className="text-[11px] text-tertiary">{unsupported}</span>
        <button className="fv-btn" onClick={onClose}>关闭</button>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3" data-testid="fv-error">
      ...
      <div ref={bodyRef} className="flex-1 overflow-auto bg-canvas">
        <Highlight theme={themes.github} code={content ?? ""} language={language}>
          {({ tokens, getLineProps, getTokenProps }) => (
            <pre className="text-[12px] font-mono m-0 p-2">
              <code>
                {tokens.map((line, i) => (
                  <div {...getLineProps({ line, key: i })} key={i} className="table-row" data-line={i + 1}>
                    <span className="table-cell pr-3 text-right text-tertiary select-none" style={{ opacity: 0.6 }}>{i + 1}</span>
