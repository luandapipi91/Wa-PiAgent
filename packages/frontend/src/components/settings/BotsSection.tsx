...
export function BotsSection() {
	...
	const { loadBots, createBot, updateBot, deleteBot } =
	...
	return (
		<div className="relative flex flex-1 min-h-0">
			{/* 左：机器人列表 */}
			<div
				className="w-56 border-r border-hairline p-3 flex flex-col gap-2"
			...
			>
				...
				{bots.map((b) => (
					<div
						...
						onKeyDown={(e) => {
							if (e.key === "Enter" || e.key === " ") openEdit(b.id);
						}}
					...
					>
						<div className="flex items-center gap-1.5 text-sm font-medium text-primary">
							...
							<button
								...
								title={
									b.enabled
										? t("settings.bot.clickDisable")
										: t("settings.bot.clickEnable")
								}
			...
			<div className="flex-1 flex flex-col gap-3 p-4 overflow-auto">
				{!draft && (
					<div className="text-sm text-tertiary p-4">
						{t("settings.bot.emptyForm")}
					</div>
				)}
				{draft && (
					<>
						...
						<div className="flex gap-3">
							<label className="flex flex-col gap-1 w-56">
								<span className="text-xs text-secondary">
									{t("settings.bot.botIdLabel")}
								</span>
								<input
									value={draft.credentials.botId}
									onChange={(e) =>
										setDraft({
											...draft,
											credentials: {
							...
							<label className="flex flex-col gap-1 w-56">
								<span className="text-xs text-secondary">
									{t("settings.bot.secretLabel")}
									{selectedId ? t("settings.bot.secretKeepEmpty") : ""}
								</span>
								<input
									type="password"
						...
						<label className="flex flex-col gap-1 w-full max-w-lg">
							...
							<span className="text-xs text-tertiary">
								{t("settings.bot.promptHint")}
							</span>
						</label>
						<label className="flex flex-col gap-1 w-72">
							<span className="text-xs text-secondary">
								{t("settings.bot.granularityLabel")}
							</span>
							<select
								value={draft.replyGranularity}
								onChange={(e) =>
									setDraft({
										...draft,
										replyGranularity: e.target.value as any,
									})
								}
								className="px-2 py-1.5 rounded-sm border border-hairline bg-surface text-sm text-primary outline-none"
						...
						<label className="flex flex-col gap-1 w-72">
							<span className="text-xs text-secondary">
								{t("settings.bot.defaultProjectLabel")}
							</span>
							<select
								value={draft.defaultProjectId}
								onChange={(e) =>
									setDraft({ ...draft, defaultProjectId: e.target.value })
								}
								className="px-2 py-1.5 rounded-sm border border-hairline bg-surface text-sm text-primary outline-none"
							...
							<span className="text-xs text-tertiary">
								{t("settings.bot.defaultProjectHint")}
							</span>
						</label>
						<div className="flex flex-col gap-1">
							<label className="flex items-center gap-2 text-sm text-secondary">
								<input
									type="checkbox"
									checked={draft.allowProjectSwitch}
									onChange={(e) =>
										setDraft({ ...draft, allowProjectSwitch: e.target.checked })
									}
							...
							<span className="text-xs text-tertiary">
								{t("settings.bot.allowSwitchHint")}
							</span>
						</div>
			...
			{confirmDelete && selectedId && (
				<ConfirmDialog
					...
					message={t("settings.bot.deleteMessage", {
						name: selected?.name ?? "",
					})}
