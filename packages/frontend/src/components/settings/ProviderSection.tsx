...
export function ProviderSection() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<ModelProvider | null>(
    null,
  );
  const [testStatuses, setTestStatuses] = useState<
    Record<
      string,
      { state: "idle" | "testing" | "ok" | "fail"; error?: string }
    >
  >({});
  ...
  return (
    <div className="flex flex-col gap-2 p-4 overflow-auto">
      ...
      {editing && (
        <ProviderFormModal initial={editing} onClose={() => setEditing(null)} />
      )}
      {confirmDelete && (
        <ConfirmDialog
          title={t("settings.provider.deleteTitle")}
          message={t("settings.provider.deleteMessage", {
