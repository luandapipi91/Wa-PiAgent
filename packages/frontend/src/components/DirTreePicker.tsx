...
export function DirTreePicker({ onPick, onCancel, showFiles = false }: Props) {
	...
	useEffect(() => {
		(async () => {
			...
			for (const [id, item] of Object.entries(items)) {
				if (id === "root") continue;
				if (home.startsWith(item.data.path)) {
					currentId = id;
					break;
				}
			}
			...
			while (segIdx < allSegments.length) {
				const entries = (
					await listDir(currentPath, showHiddenRef.current)
	...
	const handleMissingItems = useCallback(
		async (missingIds: TreeItemIndex[]) => {
			...
			for (const parentId of needLoad) {
				...
				if (!parent) continue;
				const entries = (
					await listDir(parent.data.path, showHiddenRef.current)
				).filter((e) => showHiddenRef.current || !e.name.startsWith("."));
	...
	const handleExpandItem = useCallback(
		(item: TreeItem<FsNodeData>) => {
			...
			if (!searchTreeItemsRef.current || loadedDirsRef.current.has(path))
				return;
			...
			void (async () => {
				const entries = (await listDir(path, showHiddenRef.current)).filter(
					(e) =>
						(showFiles || e.isDir) &&
						(showHiddenRef.current || !e.name.startsWith(".")),
				);
	...
	useEffect(() => {
		...
		const searchRoots = selectedPathRef.current
		...
		const rebuild = () => {
			...
			setSearchTreeItems(
				buildSearchTree(filtered, searchRoots, t("filePicker.thisPc")),
			);
		};
						...
						{isSearching && searchLoading && !hasSearchResults ? (
							...
							>
								<Tree
									treeId="dir-tree"
									rootItem="root"
									treeLabel={t("dirPicker.treeLabel")}
								/>
							</ControlledTreeEnvironment>
						)}
					</div>
