...
const zh = {
	...
	trash: {
		...
		reasonManual: "手动删除",
		reasonAuto: "自动归档",
		viewerNotice: "此会话在归档区中，为只读模式。",
		viewerRestoreLink: "恢复会话",
		viewerRestoreHint: "恢复后可继续对话",
		viewerReadonly: "只读模式 — 恢复会话后可继续对话",
		view: "查看消息",
		viewerBack: "返回归档区",
		confirmEmptyTitle: "清空归档区",
		confirmEmptyMsg:
			"确定要永久删除归档区中的全部 {{count}} 个会话吗？此操作不可撤销。",
	...
	settings: {
		...
		skill: {
			groupGlobal: "全局技能",
			groupProjectWithName: "项目技能 · {{name}}",
	...
	git: {
		...
		pullUpToDate: "已是最新",
		pullSummary:
			"{{from}} → {{to}}，{{files}} 个文件 +{{insertions}}/-{{deletions}}",
	...
	kernelMsg: {
		...
		"project.duplicateCwd": "相同目录的项目已存在",
		"project.defaultImmutable": "默认工作区不可修改",
		"project.defaultUndeletable": "默认工作区不可删除",
		"project.defaultDirCreateFailed": "默认工作区会话目录创建失败",
		"session.notFound": "会话不存在（{{sessionId}}）",
		...
		"session.executeFailed": "立即执行失败",
		"session.noValidLines": "会话文件没有有效内容，无法读取历史",
		"model.notSelected": "未选择模型，请先在模型选择器中选择一个模型",
		"model.parseFailed": "无法识别模型 {{pattern}}，请使用 provider/modelId 形式",
		"agent.notFound": "智能体不存在（{{name}}）",
		"agent.invalidDisplayName": "名称不能为空或含非法字符（{{name}}）",
		"agent.presetNotFound": "预设不存在（{{id}}）",
		"mcp.serverNotFound": "MCP 服务器 {{name}} 不存在",
		"mcp.originalServerNotFound": "原服务器 {{name}} 不存在，无法重命名",
		"mcp.configParseFailed": "MCP 配置文件解析失败",
		"memory.entryStale": "条目不存在或已被修改，请刷新列表",
		"memory.archiveNotFound": "归档条目不存在",
		"skill.dirNotFound": "目录不存在",
		...
		"attachment.unsupportedType": "不支持的文件类型：{{mime}}",
		"attachment.previewTooLarge": "文件过大（{{sizeMb}}MB 超过上限 {{maxMb}}MB）",
		"attachment.statFailed": "无法获取文件信息",
		"attachment.tooLarge": "文件超过 {{maxMb}}MB 上限",
		"extui.dialogMissing": "对话框不存在或已应答",
		"contacts.disabled": "通讯录未启用",
		"contacts.notFound": "联系人不存在（{{id}}）",
		// ===== 第三批：routes HTTP + 分享/插件/npm/scheduler B 类 code 化（37 个） =====
		"common.missingParam": "缺少参数（{{name}}）",
		"share.pathsRequired": "请先选择要分享的文件或文件夹",
		"share.invalidId": "分享 ID 非法",
		"share.nameRequired": "分享名称不能为空",
		"share.tokenMissing": "未配置分享 Token，请在 设置 → 分享 中配置",
		"share.invalidName": "分享名称含非法字符（仅限字母/数字/中文/-_./空格）",
		...
		"mcp.missingCommandOrUrl": "MCP 服务器 {{name}} 缺少 command 或 url 配置",
