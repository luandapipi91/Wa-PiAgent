...
export const useExtensionsStore = create<ExtensionsState>((set) => ({
    ...
    load: () => {
        api.get("/api/extensions")
            .then((data: any) => {
                if (data) set({ packages: data.packages ?? [], error: null });
            })
            .catch(() => {});
    },
    ...
    applyProgress: (data) =>
        set((s) => {
            if (s.upgrading[data.name] !== undefined) {
                return {
                    upgrading: { ...s.upgrading, [data.name]: data.message },
                };
            }
            const entry = s.installs[data.name];
            if (!entry) return {};
            return {
                installs: {
                    ...s.installs,
                    [data.name]: { ...entry, progress: data.message },
                },
            };
        }),
    ...
    uninstallPackage: (name) => {
        set((s) => ({
            error: null,
            uninstalling: { ...s.uninstalling, [name]: true },
        }));
