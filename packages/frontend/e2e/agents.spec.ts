...
test.describe
  .serial("多智能体矩阵关键链路", () => {
    ...
    test("1 侧边栏智能体折叠为一行：计数随智能体数更新，点击打开宫格", async ({
      page,
    }) => {
      // 初始仅 dev（global-setup 预置，displayName=研发），折叠栏计数为 1
      await expect(page.getByTestId("agent-collapsed")).toBeVisible({
        timeout: 10_000,
      });
      await expect(page.getByTestId("agent-collapsed")).toContainText("1", {
        timeout: 10_000,
      });
      ...
      await expect(page.getByTestId("agent-collapsed")).toContainText("4", {
        timeout: 10_000,
      });
    ...
    test("2 宫格弹窗：打开 → 卡片 → UI 新建 → 右键编辑进详情弹窗", async ({
      page,
    }) => {
      try {
        await page.getByTestId("agent-collapsed").click();
        ...
        await expect(page.getByTestId(`gallery-card-${UI_AGENT}`)).toBeVisible({
          timeout: 10_000,
        });
    ...
    test("3 详情弹窗：改简介 + 关系网 + 保存 → 宫格卡片简介更新", async ({
      page,
    }) => {
      ...
      await expect(page.getByTestId("partner-switch-研发")).toHaveAttribute(
        "data-on",
        "true",
      );
      // 自身开关点击无效，保持关
      await page.getByTestId(`partner-switch-${A1}`).click();
      await expect(page.getByTestId(`partner-switch-${A1}`)).toHaveAttribute(
        "data-on",
        "false",
      );
      ...
      await expect(page.getByTestId(`gallery-card-${A1}`)).toContainText(
        "E2E 矩阵简介",
        { timeout: 10_000 },
      );
      ...
      await expect(
        cfg.locator("label", { hasText: "简介" }).locator("input"),
      ).toHaveValue("E2E 矩阵简介");
      await page.getByTestId("tab-partners").click();
      await expect(page.getByTestId("partner-switch-研发")).toHaveAttribute(
        "data-on",
        "true",
    ...
    test("4 左键智能体 → 新建会话页预选 → 发消息 → pill 为该智能体", async ({
      ...
      const testid = await page
        .locator('aside [data-testid^="session-"]:has-text("矩阵链路消息")')
        .first()
    ...
    test("6 Composer：@ 出智能体补全、# 出文件补全", async ({ page }) => {
      ...
      mkdirSync(projectCwd, { recursive: true });
      writeFileSync(
        join(projectCwd, "matrix-file.txt"),
        "agent matrix e2e",
        "utf8",
      );
      ...
      await expect(
        page.locator('[data-testid="composer-input"] .chip-agent').first(),
    ...
    test("7 删除智能体：二次确认 → 列表消失；会话保留 → agent_missing 重选恢复", async ({
      page,
    }) => {
      try {
        ...
        await expect(page.getByTestId(`gallery-card-${A2}`)).toHaveCount(0, {
          timeout: 10_000,
        });
        ...
        await expect(page.getByTestId("switcher-missing")).toBeVisible({
          timeout: 10_000,
        });
        ...
        await page.getByTestId("composer-send").click();
        await expect(page.getByTestId("agent-missing-modal")).toBeVisible({
          timeout: 10_000,
        });
        ...
        await expect(page.getByTestId("agent-switcher")).toContainText("研发", {
          timeout: 10_000,
        });
        await expect(page.getByText("已切换为 研发")).toBeVisible({
          timeout: 10_000,
        });
      } finally {
...
test.describe
  .serial("技能 tab：全部勾选开关与描述气泡", () => {
    ...
    test("全部勾选开关：默认 ON → 点击全不选 → 再次点击恢复全选；描述点击气泡", async ({
      page,
    }) => {
      await page.goto("/", { timeout: 60_000 });
      test.setTimeout(120_000);
      ...
      await page.getByTestId("agent-collapsed").click();
      await page
        .getByTestId(`gallery-card-${SK_AGENT}`)
        .click({ button: "right" });
      await page.getByTestId("gallery-ctx-edit").click();
      await expect(page.getByTestId("agent-config")).toBeVisible();
      await expect(page.getByTestId("cfg-name-input")).toHaveValue(SK_AGENT, {
        timeout: 10_000,
      });
      ...
      // 等待测试技能出现（store 异步加载）
      await expect(page.getByTestId("skill-switch-e2e-skill-long")).toBeVisible(
        { timeout: 10_000 },
      );

      // 全部勾选开关默认 ON
      await expect(page.getByTestId("skill-select-all")).toHaveAttribute(
        "data-on",
        "true",
      );
      await expect(
        page.getByTestId("skill-switch-e2e-skill-long"),
      ).toHaveAttribute("data-on", "true");
      await expect(
        page.getByTestId("skill-switch-e2e-skill-short"),
      ).toHaveAttribute("data-on", "true");
      ...
      await expect(
        page.getByTestId("skill-desc-bubble-e2e-skill-long"),
      ).toHaveCount(0);
