...
test.describe.serial("Git 分支管理", () => {
	...
	test("Git 工具栏渲染 + 分支下拉搜索与切换", async ({ page }) => {
		...
		// 工具栏：拉取按钮 + 项目 chip + 分支 chip（当前 master）
		await expect(page.getByTestId("btn-git-pull")).toBeVisible();
		await expect(page.getByTestId("git-project-chip")).toContainText(
			PROJECT_NAME,
		);
	...
	test("创建并检出新分支（非法名禁用提交）", async ({ page }) => {
		...
		await expect(
			page.getByTestId("btn-create-branch-confirm"),
		).toBeDisabled();
	...
	test("Git 图谱展示提交历史与装饰标签", async ({ page }) => {
		...
		await expect(modal.locator('[data-testid^="git-log-row-"]')).toHaveCount(
			3,
			{ timeout: 8000 },
		);
	...
	test("无上游仓库 pull 失败给出错误提示", async ({ page }) => {
		...
		await page.getByTestId("btn-git-pull").click();
		// 无 origin/上游 → kernel 返回 git.pullFailed → toast 容器出现错误提示
