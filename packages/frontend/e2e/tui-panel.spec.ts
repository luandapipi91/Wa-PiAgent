...
test.describe.serial("扩展 TUI 面板（ctx.ui.custom）", () => {
	let projectId = "";
	let projectName = "";
	...
	test("面板三态：收起为挂件、再收为胶囊、点胶囊展开", async ({ page }) => {
		...
		// 「—」收起 → 挂件卡片（标题栏第一个按钮）
		await page.getByTestId("tui-panel-header").getByRole("button").first().click();
	...
	test("收起态下输入框恢复可用", async ({ page }) => {
		...
		await page.getByTestId("tui-panel-header").getByRole("button").first().click();
		await expect(page.getByTestId("tui-panel-badge")).toBeVisible({ timeout: 10_000 });
		await page.getByTestId("tui-panel-badge").getByRole("button").click();
		await expect(page.getByTestId("tui-panel-pill")).toBeVisible({ timeout: 10_000 });
