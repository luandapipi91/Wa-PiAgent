...
test("聊天块渲染：工具卡弱化折叠 + 代码块卡片 + FilePill 预览 + 正文可见", async ({
	page,
}) => {
	...
	await expect(toolCard).toHaveAttribute("data-muted", "true", {
		timeout: 180_000,
	});
...
test("代码卡片内 prism table token 不被 Tailwind .table 工具类竖排", async ({
	page,
}) => {
	await page.goto("/");
