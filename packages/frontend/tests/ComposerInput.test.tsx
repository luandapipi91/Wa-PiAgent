...
const fetchMock = mock((input: RequestInfo | URL, init?: RequestInit) => {
	const url =
		typeof input === "string" ? input : ((input as any).href ?? String(input));
	const form = init?.body as FormData | undefined;
	const file =
		form && typeof form.get === "function"
			? (form.get("file") as File | undefined)
			: undefined;
	fetchCalls.push({ url, fileName: file?.name });
	const projectId =
		new URL(url, "http://localhost").searchParams.get("projectId") ?? "p1";
...
test("#文件搜索结果中目录项传递 isDir 并显示文件夹图标", async () => {
	...
	await waitFor(() => {
		req = apiCalls.find(
			(c) => c.path === "/api/fs/search" && c.body?.query === "src",
		);
...
test("选中文件后生成 #[path] chip token", async () => {
	...
	await waitFor(() => {
		req = apiCalls.find(
			(c) => c.path === "/api/fs/search" && c.body?.query === "hello",
		);
		expect(req).toBeTruthy();
	});
...
test("发送时 chip token 展开为纯文本", () => {
	...
	useSkillsStore.setState({
		allSkills: [
			{ name: "pdf", description: "", path: "/s", source: { type: "builtin" } },
		],
		skills: [],
		dirs: [],
		disabledSkills: [],
...
describe("ComposerInput @ 候选菜单过滤", () => {
	beforeEach(() => {
		...
		useProjectsStore.setState({
			projects: [{ id: "p1", name: "t", cwd: "/tmp", createdAt: 0 }],
			currentProjectId: "p1",
		});
	});
...
describe("超大附件降级处理", () => {
	...
	it("Electron 环境：>50MB 文件降级为路径引用（不报错、不上传）", async () => {
		(window as any).waPiApp = {
			getPathForFile: (f: File) => `/home/user/${f.name}`,
		};
		const setAttachments = mock() as any;
	...
	it("浏览器环境：>50MB 文件提示超限（无 waPiApp）", async () => {
		...
		// 显示超限提示，不添加附件
		await waitFor(() =>
			expect(screen.getByText(/超过.*50MB.*上限/)).toBeTruthy(),
		);
	...
	it("Electron 环境：≤50MB 文件仍正常上传（不降级）", async () => {
		...
		await act(async () => {
			fireEvent.change(input, {
				target: { files: [bigFile("small.txt", 10)] },
			});
		});
