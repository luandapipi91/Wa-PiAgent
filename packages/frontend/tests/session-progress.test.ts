...
test("handleSubagentProgress：delegate 单 agent 存入并按 [toolCallId][agent] 索引", () => {
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-a"));
...
test("handleSubagentProgress：同 toolCallId 同 agent 二次写入 = 更新（而非追加）", () => {
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-a", { output: "v1" }));
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-a", { output: "v2" }));
	const byAgent = useSessionStore.getState().progressByToolCall["tc1"];
...
test("handleSubagentProgress：fleet 多 agent 共享同一 toolCallId 并存", () => {
	...
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-b"));
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-c"));
	const byAgent = useSessionStore.getState().progressByToolCall["tc1"];
...
test("handleSubagentProgress：不同 toolCallId 隔离", () => {
	...
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc2", progress("agent-b"));
...
// ── 同名 agent 任务隔离（taskIndex）：同名不互相覆盖 ──

// B1：同名 agent 不同 taskIndex 各自独立存储（fleet 同名多任务不互相覆盖）
test("handleSubagentProgress：同名 agent 不同 taskIndex 各自独立存储", () => {
	useSessionStore.getState().handleSubagentProgress(
		"s1",
		"tc1",
		progress("Explore", {
			taskIndex: 0,
	...
	const byKey = useSessionStore.getState().progressByToolCall["tc1"];
	// 两个同名 agent 任务各自独立 key，不互相覆盖
	expect(Object.keys(byKey).sort()).toEqual(["0", "1"]);
	expect(byKey["0"].tools).toHaveLength(1);
	expect(byKey["1"].tools).toHaveLength(2);
});

// B2：无 taskIndex（delegate 单任务 / 老数据）按 agent 名降级 key
test("handleSubagentProgress：无 taskIndex 时按 agent 名降级 key（向后兼容）", () => {
	useSessionStore
		.getState()
		.handleSubagentProgress("s1", "tc1", progress("agent-a"));
	const byKey = useSessionStore.getState().progressByToolCall["tc1"];
	expect(byKey["agent-a"]).toBeDefined();
	expect(Object.keys(byKey)).toEqual(["agent-a"]);
});
