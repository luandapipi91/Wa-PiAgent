...
describe("fs-client listDir", () => {
	...
	test("showHidden 透传到请求", async () => {
		fake.setResponse("fs:listDir", {
			entries: [{ name: ".hidden", isDir: true }],
		});
...
describe("fs-client uploadFile", () => {
	...
	test("resolves with path on multipart upload result", async () => {
		mockFetch({
			ok: true,
			data: { path: "/project/p1/.wa-pi/uploads/img.png" },
		});
	...
	test("rejects on upload error", async () => {
		...
		await expect(
			uploadFile("p1", "img.png", new Blob(["base64data"])),
...
describe("fs-client openFileWithDefaultApp", () => {
	beforeEach(() => {
		fake.calls.length = 0;
		fake.sent.length = 0;
		fake.responses.clear();
	...
	test("发送 /api/fs/open-with-default-app 请求", async () => {
		fake.setResponse("fs:openWithDefaultApp", {});
		const { openFileWithDefaultApp } = await import("../src/fs-client");
		await openFileWithDefaultApp("/work/demo/a.zip");
		expect(fake.calls[0]).toMatchObject({
			method: "POST",
			path: "/api/fs/open-with-default-app",
			body: { path: "/work/demo/a.zip" },
		});
	});
});
