...
test("挂载时请求 agent:list；收到 agent:list 事件写入 agents store", () => {
  ...
  handler!({ type: "agent:list", agents: [agent("dev", "技术实现")] });
  expect(useAgentsStore.getState().list.map(a => a.name)).toEqual(["dev"]);
});

test("侧栏点智能体 → 新建会话视图且下拉预选该智能体", async () => {
  useAgentsStore.setState({ list: [agent("dev", "技术实现"), agent("代码审查")] });
  render(<App />);
  // 初始默认第一项
...
test("侧栏右键「编辑智能体」→ 打开 AgentConfig 弹窗", async () => {
  useAgentsStore.setState({ list: [agent("dev", "技术实现")] });
  render(<App />);
  fireEvent.contextMenu(screen.getByTestId("agent-dev"));
