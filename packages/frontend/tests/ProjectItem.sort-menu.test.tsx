...
test("点击「重命名」弹出内置弹窗，输入框预填当前标题", () => {
	renderIt();
	fireEvent.contextMenu(screen.getByTestId("session-new"));
	fireEvent.click(screen.getByTestId("menu-rename"));
	// popup 关闭、内置弹窗打开（非原生 prompt）
	expect(screen.queryByTestId("session-context-menu")).toBeNull();
...
test("重命名弹窗点确认 → 调用 POST /api/sessions/:id/rename 并关闭", () => {
	renderIt();
	fireEvent.contextMenu(screen.getByTestId("session-old"));
	fireEvent.click(screen.getByTestId("menu-rename"));
	...
	expect(apiCalls).toContainEqual({
		method: "post",
		path: "/api/sessions/old/rename",
		body: { title: "改后标题" },
	});
	expect(screen.queryByTestId("rename-dialog")).toBeNull();
});

test("重命名弹窗点取消 → 不发送请求并关闭", () => {
	renderIt();
	fireEvent.contextMenu(screen.getByTestId("session-old"));
	fireEvent.click(screen.getByTestId("menu-rename"));
