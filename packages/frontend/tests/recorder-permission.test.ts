...
import { _setRecordingManager, toRecordingErrorMessage } from "../src/recording/recorder";
...
test("RecordingManager.start：getUserMedia 权限拒绝 → 抛业务文案（非原始英文）", async () => {
  ...
  Object.defineProperty(navigator, "mediaDevices", {
    value: {
      ...
      getDisplayMedia: async () => { throw new Error("not called"); },
    },
    configurable: true,
  });
  try {
    ...
    await expect(
      getRecordingManager().start({ source: "mic", projectId: "p1", sessionId: "s1", ownerLabel: "x", onTick: () => {} }),
    ).rejects.toThrow(/权限/);
  } finally {
