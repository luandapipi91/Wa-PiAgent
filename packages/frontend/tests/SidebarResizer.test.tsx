...
test("渲染分隔条，cursor 为 col-resize", () => {
  render(<SidebarResizer side="left" onResize={() => {}} testId="r" />);
...
// ── 左侧（side=left）：向右拖增宽，width = clientX ──

test("[left] mousedown + mousemove 向右拖 → 宽度 = clientX", () => {
  const onResize = mock();
  render(<SidebarResizer side="left" onResize={onResize} testId="r" />);
...
// ── 右侧（side=right）：向左拖增宽，width = innerWidth - clientX ──

test("[right] mousedown + mousemove 向左拖 → 宽度 = innerWidth - clientX", () => {
  const onResize = mock();
  render(<SidebarResizer side="right" onResize={onResize} testId="r" />);
  // 面板宽 320，右边缘在 clientX=680（innerWidth=1000）；向左拖到 clientX=650 → 宽度 350
...
test("[right] 向右拖（缩小）→ 宽度减小", () => {
  ...
  render(<SidebarResizer side="right" onResize={onResize} testId="r" />);
  fireEvent.mouseDown(screen.getByTestId("r"), { clientX: 680 });
  window.dispatchEvent(new MouseEvent("mousemove", { clientX: 720 })); // innerWidth-720=280
  fireEvent.mouseUp(screen.getByTestId("r"));
...
test("[right] 拖超 40% → 钳制到 400", () => {
  ...
  render(<SidebarResizer side="right" onResize={onResize} testId="r" />);
  fireEvent.mouseDown(screen.getByTestId("r"), { clientX: 680 });
  // 向左拖很远：clientX=200 → raw=800，超过 400 上限
...
test("[right] 拖低于 200 → 钳制到 200", () => {
  ...
  render(<SidebarResizer side="right" onResize={onResize} testId="r" />);
  fireEvent.mouseDown(screen.getByTestId("r"), { clientX: 680 });
  // 向右拖很远：clientX=850 → raw=150，低于 200 下限
