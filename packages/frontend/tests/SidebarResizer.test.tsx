...
test("渲染分隔条，cursor 为 col-resize", () => {
  render(<SidebarResizer side="left" getWidth={() => 264} onResize={() => {}} testId="r" />);
...
test("[left] mousedown + mousemove 向右拖 → 起始宽 + 增量", () => {
  const onResize = mock();
  render(<SidebarResizer side="left" getWidth={() => 264} onResize={onResize} testId="r" />);
...
test("[right] mousedown + mousemove 向左拖 → 起始宽 - 增量", () => {
  const onResize = mock();
  render(<SidebarResizer side="right" getWidth={() => 320} onResize={onResize} testId="r" />);
