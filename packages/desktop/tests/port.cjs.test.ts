...
test("killPortOccupants: windows 用 netstat 解析 PID 后 taskkill", async () => {
  ...
  const fakeSpawn = mock((cmd: string, args: string[]) => {
    ...
    if (cmd === "netstat") {
      return {
        stdout: "  TCP    127.0.0.1:59777   0.0.0.0:0  LISTENING  3333\r\n  TCP    0.0.0.0:9777  0.0.0.0:0  LISTENING  9999\r\n",
        status: 0,
      };
    }
    return { stdout: "", status: 0 };
...
test("killPortOccupants: netstat PID 是死进程（幽灵占用）→ 回退扫描并杀掉带 wa-pi 特征的存活进程", async () => {
  ...
  const fakeSpawn = mock((cmd: string, args: string[]) => {
    calls.push({ cmd, args });
    if (cmd === "netstat") {
      return { stdout: "  TCP    0.0.0.0:59778  0.0.0.0:0  LISTENING  30000\r\n", status: 0 };
    }
    if (cmd === "powershell") {
      // 幽灵扫描发现一个存活的我方 kernel（命令行含数据目录路径）
      return {
        stdout: JSON.stringify([
          { ProcessId: 28788, ParentProcessId: 30000, CommandLine: "bun run C:\\wa-pi-ghost\\runtime\\kernel.js" },
        ]),
        status: 0,
      };
    }
  ...
  try {
    ...
    expect(calls.some((c) => c.cmd === "taskkill" && c.args.includes("30000"))).toBe(true);
    expect(calls.some((c) => c.cmd === "taskkill" && c.args.includes("28788"))).toBe(true);
    expect(pids).toEqual([30000, 28788]);
    // 被杀 PID 记录日志（含命令行摘要）
    expect(logs.some((m) => m.includes("28788") && m.includes("kernel.js"))).toBe(true);
  } finally {
...
test("killPortOccupants: 幽灵扫描不误杀 CLI 模式 / 其他数据目录的进程", async () => {
  ...
  const fakeSpawn = mock((cmd: string, args: string[]) => {
    calls.push({ cmd, args });
    if (cmd === "netstat") return { stdout: "  TCP    0.0.0.0:59779  0.0.0.0:0  LISTENING  30000\r\n", status: 0 };
    if (cmd === "powershell") {
      return {
        stdout: JSON.stringify([
          // 我方 seed（含数据目录路径）→ 应杀
          { ProcessId: 5000, ParentProcessId: 1, CommandLine: "bun run C:\\wa-pi-ghost\\runtime\\kernel.js" },
          // seed 的子孙（命令行无特征，进程树关联）→ 应杀
          { ProcessId: 5001, ParentProcessId: 5000, CommandLine: "cmd.exe /c some-background-task" },
          // 其他数据目录的 pi（其他工作区）→ 不杀
          { ProcessId: 6000, ParentProcessId: 1, CommandLine: "node C:\\pi\\cli.js --session C:\\wa-pi-other\\sessions\\x.jsonl" },
          // 无关进程的子孙 → 不杀
          { ProcessId: 6001, ParentProcessId: 6000, CommandLine: "node server.js" },
          // CLI 模式 pi（仅含 pi-coding-agent 字样，与我方数据目录无关）→ 不杀
          { ProcessId: 7000, ParentProcessId: 1, CommandLine: "node C:\\nvm\\pi-coding-agent\\dist\\cli.js" },
        ]),
        status: 0,
      };
    }
  ...
  try {
    ...
    for (const safe of ["6000", "6001", "7000"]) {
      expect(calls.some((c) => c.cmd === "taskkill" && c.args.includes(safe))).toBe(false);
    }
  } finally {
    if (savedDir === undefined) delete process.env.WA_PI_DIR;
    else process.env.WA_PI_DIR = savedDir;
...
test("killPortOccupants: 幽灵扫描也找不到我方进程 → 不抛错，返回已杀 PID", async () => {
  ...
  const fakeSpawn = mock((cmd: string) => {
    if (cmd === "netstat") return { stdout: "  TCP    0.0.0.0:59780  0.0.0.0:0  LISTENING  30000\r\n", status: 0 };
...
test("killPortOccupants: taskkill 失败（权限不足/死 PID）不计入成功列表，失败 PID 与端口结果输出日志", async () => {
  ...
  const fakeSpawn = mock((cmd: string, args: string[]) => {
    calls.push({ cmd, args });
    if (cmd === "netstat") {
      return { stdout: "  TCP    0.0.0.0:59776  0.0.0.0:0  LISTENING  3333\r\n", status: 0 };
    }
  ...
  expect(logs.some((m) => m.includes("清理结果") && m.includes("3333") && m.includes("已释放"))).toBe(true);
});
...
test("killPortOccupants: 同前缀兄弟目录（如 wa-pi-ghost2）的进程不误杀", async () => {
  ...
  const fakeSpawn = mock((cmd: string, args: string[]) => {
    calls.push({ cmd, args });
    if (cmd === "netstat") return { stdout: "  TCP    0.0.0.0:59781  0.0.0.0:0  LISTENING  30000\r\n", status: 0 };
    if (cmd === "powershell") {
      return {
        stdout: JSON.stringify([
          // 我方 seed（数据目录恰好是 C:\wa-pi-ghost）→ 应杀
          { ProcessId: 9000, ParentProcessId: 1, CommandLine: "bun run C:\\wa-pi-ghost\\runtime\\kernel.js" },
          // 同前缀兄弟目录 C:\wa-pi-ghost2 → 不杀（缺结尾边界时会误判为 seed）
          { ProcessId: 8000, ParentProcessId: 1, CommandLine: "bun run C:\\wa-pi-ghost2\\runtime\\kernel.js" },
          // 兄弟目录进程的子孙 → 不杀
          { ProcessId: 8001, ParentProcessId: 8000, CommandLine: "node server.js" },
        ]),
        status: 0,
      };
    }
