...
test("pet.html：多屏偏移不漏算（首启位置与缩放 clamp 都带 virt 边界）", () => {
	...
	expect(html).toContain("st.fx = virt.r - 230; st.fy = virt.b - 60;");
	expect(html).not.toContain("st.fx = sw - 230");
	// 缩放 clamp 同样要带 virt.l / virt.t 偏移，否则会把宠物抓回主屏左边缘
	expect(html).toContain("st.fx = clamp(st.fx, virt.l + 90, virt.r - 90);");
	expect(html).toContain("st.fy = clamp(st.fy, virt.t + 60, virt.b - 12);");
});
