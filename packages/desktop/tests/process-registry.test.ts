...
// 用 path.join 构造，使 WAPI_DIR 与 path.join 派生路径（entry.exe 等）分隔符一致；
// 否则 Windows 上 WAPI_DIR 仍是正斜杠而派生路径被转成反斜杠，isOurs 的 exe.includes(dir) 永远 false
const WAPI_DIR = path.join("/data", "wa-pi");
...
function makeMemFs(seed: Record<string, string> = {}) {
	...
	return {
		...
		readdirSync(p: string): string[] {
			// 跨平台：path.join 在 Windows 产生反斜杠 key，分隔符须用 path.sep 而非硬编码 "/"
			const prefix = String(p).replace(/[\\/]+$/, "");
			return [...store.keys()]
				.filter((k) => k.startsWith(prefix + path.sep))
				.map((k) => k.slice(prefix.length + path.sep.length))
				.filter((n) => !n.includes(path.sep) && !n.includes("/"));
		},
...
function makeOpts(overrides: any = {}) {
	...
	const spawnSync = mock((cmd: string, args: string[]) => {
		...
		return overrides.spawnResult
			? overrides.spawnResult(cmd, args)
			: { stdout: "", status: 0 };
	}) as any;
...
test("sweepRegistry: 超 TTL → 只删文件不杀（不查身份不杀进程）", () => {
	...
	const ctx = makeOpts({
		seed: { [path.join(REG_DIR, "1001.json")]: JSON.stringify(expired) },
	});
	...
	expect(ctx.fs.store.has(path.join(REG_DIR, "1001.json"))).toBe(false);
	expect(
		ctx.spawnCalls.some((c) =>
			["powershell", "ps", "taskkill"].includes(c.cmd),
		),
	).toBe(false);
...
test("getProcessIdentity: Windows 走 PowerShell 解析 CreationDate/ExecutablePath", () => {
	...
	expect(id).toEqual({
		ok: true,
		identity: {
			exe: "C:\\wa-pi\\kernel.exe",
			createdAt: Date.parse("2023-11-14T22:13:20.123+08:00"),
		},
	});
	...
	expect(ps!.args).toEqual([
		...
		expect.stringContaining(
			'Get-CimInstance Win32_Process -Filter "ProcessId=555"',
		) as any,
	]);
...
test("getProcessIdentity: Windows 进程不存在（stdout 空）→ not-found 结果", () => {
	...
	expect(getProcessIdentity(999999, ctx.opts)).toEqual({
		ok: false,
		reason: "not-found",
	});
});
...
test("getProcessIdentity: 非 Windows 走 ps 解析 lstart，exe 取 command 首 token", () => {
	const ctx = makeOpts({
		...
		spawnResult: (cmd: string) =>
			cmd === "ps"
				? {
						stdout:
							"Mon Aug 10 14:48:27 2026     /data/wa-pi/runtime/kernel.exe run kernel.js\n",
						status: 0,
					}
				: { stdout: "", status: 0 },
	});
	...
	expect(id).toEqual({
		ok: true,
		identity: {
			exe: "/data/wa-pi/runtime/kernel.exe",
			createdAt: new Date(2026, 7, 10, 14, 48, 27).getTime(),
		},
	});
...
test("sweepRegistry: 身份查询失败 → 保留登记 + errors 可观测 + 日志记录，不杀进程", () => {
	...
	expect(r.skipped).toEqual([]);
	expect(r.errors).toEqual([
		{ pid: 1001, reason: expect.stringContaining("命令执行失败") as any },
	]);
	expect(
		ctx.logs.some((m) => m.includes("1001") && m.includes("身份查询失败")),
	).toBe(true);
...
test("sweepRegistry: 进程不存在（ps 空输出 + status 1）→ 正常 deleted 路径不受影响", () => {
	const ctx = makeOpts({
		...
		seed: seedOne(),
		spawnResult: (cmd: string) =>
			cmd === "ps" ? { stdout: "", status: 1 } : { stdout: "", status: 0 },
	});
	const r = sweepRegistry(ctx.opts);
	expect(r.deleted).toEqual([1001]);
	expect(r.errors).toEqual([]);
...
test("isOurs: ②③ 组合判断——创建时间一致且 exe 含我方特征 → true，任一不符 → false", () => {
	...
	expect(isOurs(entry, { exe: entry1001.exe, createdAt: NOW }, opts)).toBe(
		true,
	);
	expect(
		isOurs(entry, { exe: entry1001.exe, createdAt: NOW + 86400000 }, opts),
	).toBe(false); // ②创建时间不符
	expect(isOurs(entry, { exe: "/usr/bin/other", createdAt: NOW }, opts)).toBe(
		false,
	); // ③exe 不符
...
test("collectDescendants: 排除 selfPid，且不遍历其子树", () => {
	...
	expect(collectDescendants([1001], procs, 2002).map((p) => p.pid)).toEqual([
		2001,
	]);
});
...
test("killRegisteredProcesses: scanProcesses 返回 []（非 Windows/查询失败）→ 与现状一致，只杀 root", () => {
	...
	expect(r.skipped).toEqual([]);
	expect(
		ctx.spawnCalls.filter((c) => c.cmd === "taskkill").map((c) => c.args[1]),
	).toEqual(["1001"]);
});
...
test("killRegisteredProcesses: 子孙在进程表但已死（taskkill 非 0）→ 记 skipped，不阻断杀 root", () => {
	...
	const ctx = makeOpts({
		...
		spawnResult: (cmd: string, args: string[]) => {
			if (cmd === "powershell") return identityResult(NOW)(cmd);
			if (cmd === "taskkill" && args[1] === "2001")
				return { stdout: "", status: 128 }; // 进程已死
			return { stdout: "", status: 0 };
		},
	});
	ctx.opts.scanProcesses = () => procs;
