# pi-dynamic-workflows 评估与 HiAgent 的 Pi 扩展复用原则
...
HiAgent 在 2026-07-05 ~ 2026-07-17 的 Pi 生态调研中，只在 `2026-07-05-hiagent-design.md:27` 列出过 pi-dynamic-workflows 的名字（作为"成熟编排后端"之一），但：
...
`@gotgenes/pi-subagents` 内置 `applyRecursionGuard` 硬删除子智能体的 delegate 工具，导致 HiAgent 当前深度恒为 1。
...
效果上等价于 A→B→C 链式，机制上是**编排器组合**而非**子智能体递归**——`applyRecursionGuard` 不触发。这是 HiAgent 后期实现链式委托的一条可选路径（无需 fork pi-subagents）。

## 三、为什么 HiAgent 不能直接用

### 3.1 关键事实：它是「交互层扩展」，不是「底层服务」

HiAgent 能用 `@gotgenes/pi-subagents`，是因为只用了它的**底层 service API**：

```ts
// HiAgent delegate-tool.ts:130-135 —— 只用底层 spawn service
const { getSubagentsService } = await import("@gotgenes/pi-subagents");
const svc = getSubagentsService();
id = svc.spawn(agent, task);
```

HiAgent 自己包了 `delegate` customTool + DelegateCard UI + partners allowlist——**pi-subagents 的交互层被完全绕过**。

但 pi-dynamic-workflows **没有底层 service API 可借**，它的全部能力都在交互层：
- `/workflows` slash command（Pi CLI 命令，HiAgent 的 Composer 不认）
- TUI 进度面板（Pi CLI 的 TUI 渲染，HiAgent 的 React UI 看不到）
- keyword trigger（依赖 Pi CLI 的消息提交层，HiAgent Composer 不接管）
- `~/.pi/workflows/` 状态目录（HiAgent 不读也不展示）
- LLM 生成 JS 脚本 + vm 沙箱执行（需要 Pi 的工具调用机制配合，HiAgent 的 delegate 工具是另一套）

### 3.2 用户在 HiAgent 里无法"启用"它的编排

它的所有交互入口在 HiAgent UI 里一个都不存在：

| pi-dynamic-workflows 入口 | HiAgent 里是否工作 |
|---|---|
| `/workflows run <prompt>` slash command | ❌ HiAgent Composer 不解析 Pi slash command |
| `/code-review` / `/deep-research` 内置命令 | ❌ 同上 |
| TUI 进度面板 | ❌ HiAgent 是 React UI，不是 Pi CLI TUI |
| "workflow" keyword trigger | ❌ 依赖 Pi CLI 消息提交层 |
| `~/.pi/workflows/` 状态 | ❌ HiAgent 状态在 `~/.hiagent/`，不读 `.pi/` |

即使用户在 HiAgent 里 `pi install npm:@quintinshaw/pi-dynamic-workflows`，扩展加载了但**所有入口都不工作**——除非 HiAgent 做大量适配工作，把它的 TUI/命令/状态都翻译到 HiAgent 的 UI/状态模型里。

## 四、HiAgent 的 Pi 扩展复用原则（本次确立）

这次评估暴露了一个应该明确的原则：

> **HiAgent 只能复用「工具类 / 底层服务类」Pi 扩展，不能复用「交互类」Pi 扩展。**

HiAgent 虽然基于 Pi SDK 构建，但它是独立产品（自己的 kernel / frontend / desktop shell / 会话模型 / UI 渲染层）。Pi 扩展能在 HiAgent 里工作的前提是：**它的能力通过工具调用或 service API 暴露，而不是通过 Pi CLI 的 slash command / TUI / keyword trigger 暴露**。

### 4.1 分类对照

| 类型 | 特征 | HiAgent 能否复用 | 已复用示例 |
|---|---|---|---|
...
2. **它的状态是否写在 HiAgent 能读到的位置（`~/.hiagent/` 或项目内）？**
   - 是 → 可复用
   - 否，写在 `~/.pi/` 独立目录 → 状态分裂，需要适配
3. **它的 UI 渲染是否依赖 Pi CLI 的 TUI 层？**
   - 否（纯工具/service）→ 可复用
   - 是（TUI/slash command/keyword trigger）→ HiAgent UI 层不接管

## 五、HiAgent 后期多智能体编排的可行路径

既然 pi-dynamic-workflows 不可直接用，HiAgent 要支持编排本质上只有两条路：

| 路径 | 说明 | 成本 | 风险 |
|---|---|---|---|
| **(a) 自研编排层** | HiAgent 在 `delegate` 工具之上自研编排（fleet 工具 + 阶段化 UI + 状态管理），借鉴 pi-dynamic-workflows 的 `parallel`/`pipeline`/`verify`/`resume` 设计 | 中高，但完全自主 | 自研负担 |
| **(b) 深度适配 pi-dynamic-workflows** | 把它的 TUI/命令/状态全部翻译到 HiAgent UI 层（重写它的交互入口） | 极高，可能比 (a) 还大 | 绑死第三方包，未来方向不可控 |

### 5.1 推荐路径：(a) 自研编排层

**理由**：
1. HiAgent 的核心定位是「关系网轻量委托」（partners.askTo），编排是这之上的自然延伸，自主可控
2. 当前 spec 的 B3「fleet 并行委托」已经是 (a) 的第一步——在 delegate 之上加编排能力
3. pi-dynamic-workflows 的设计思路（parallel/pipeline/verify/resume）可以**借鉴但不依赖**——HiAgent 在自己的 delegate/fleet 工具 + React UI + ~/.hiagent 状态模型里实现等价能力

### 5.2 自研编排层的设计借鉴清单

从 pi-dynamic-workflows 借鉴的设计点（仅供 HiAgent 后续编排 spec 参考）：

| pi-dynamic-workflows 设计 | HiAgent 可借鉴形态 |
|---|---|
...
1. **pi-dynamic-workflows 不可直接用于 HiAgent**——它是交互类扩展，所有入口在 HiAgent UI 里不工作
...
- `docs/superpowers/specs/2026-07-05-hiagent-design.md` —— HiAgent 顶层设计
