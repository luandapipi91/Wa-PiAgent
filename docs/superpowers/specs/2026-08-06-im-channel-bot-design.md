...
4. 渠道可配置额外系统提示词，追加拼接到已有系统提示词中、**位于记忆内容之前**；编辑时支持 `$` 触发技能自动补全（插入 `$[技能名]` 引用，注入时内联展开为技能全文——见 §8）
...
**v1 做**：仅企业微信渠道（UI 预留微信/飞书/QQ 置灰「敬请期待」）；进站文本 + 图片；出站 markdown 文本（非流式）；多机器人多开；智能体绑定；工作区指令切换；渠道附加提示词（支持 `$` 技能引用，自动补全 + 注入时内联展开）；简洁/标准两档回复粒度。
...
**`$` 技能引用展开**：`extraSystemPrompt` 中可写 `$[技能名]` token（与主输入框 quick-invoke 的 token 格式一致，`packages/frontend/src/quick-invoke/tokens.ts`）。SDK 的 `/skill:name` 展开只作用于用户消息文本，对 `--system-prompt` 注入的内容**不生效**，因此 kernel 在每次入站消息准备 `imChannelContext` 前自行展开：`$[name]` → 按名查 `SkillManager` 技能 → 内联为 `<skill name="X" location="Y">SKILL.md 全文</skill>` XML 块（仿 SDK 的展开格式）；找不到的技能**保留 `$[name]` 原文**（不静默丢失）。展开逐消息进行（技能内容变更无需重启渠道；运行中的会话在下次重建后生效）。注意这与「绑定智能体已配置的 skills」（`AgentConfig.skills`，会话级能力加载）是两层东西：`$` 引用是把技能提示词文本并入系统提示词，不改变会话的技能加载列表。
