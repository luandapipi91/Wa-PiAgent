...
```bash
cd /path/to/WaPi && bun test packages/kernel/tests/agent-manager.test.ts
```
预期：PASS（含新增测试）
...
```bash
cd /path/to/WaPi/packages/frontend && bun test tests/store/session.test.ts
```
预期：PASS
...
```bash
cd /path/to/WaPi/packages/frontend && bun test tests/SessionView.test.tsx
```
预期：PASS
