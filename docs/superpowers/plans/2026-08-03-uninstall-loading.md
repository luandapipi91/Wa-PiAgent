...
- [x] **更新 CHANGELOG.md**：在顶部 `## 2026-08-03` 下新增一条（fix 类型），记录卸载等待反馈，影响范围 `packages/frontend/src/store/extensions.ts`、`packages/frontend/src/components/settings/ExtensionSection.tsx`、两个测试文件，验证方式（store 单测 + 组件测试）。
...
- [x] **手工验证**（可选）：运行 `bun run dev` 打开设置 → 插件面板，卸载一个已装插件，确认按钮转圈「卸载中…」→ 完成后卡片消失。
